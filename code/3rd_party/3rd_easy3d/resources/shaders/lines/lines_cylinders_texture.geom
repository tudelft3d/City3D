...
in vec2_  vOutTexcoord[];
in float vOutClipped[];

out vec2_  gOutTexcoord;
...
void main()
{
	vec3_  base = gl_in[0].gl_Position.xyz;  // in camera space
	vec3_  top = gl_in[1].gl_Position.xyz;   // in camera space
	vec3_  len_dir = top - base;
	vec3_  view_dir = vec3_(0,0,1);	// this is view direction for orthographic mode in camera space
	if (perspective) {
		//view_dir = normalize(vec3_(0) - 0.5*(base + top));	// camera pos is (0, 0, 0) in camera space
		view_dir = normalize(vec3_(0) - base);	// camera pos is (0, 0, 0) in camera space
	}

	vec2_ base_texcoord = vOutTexcoord[0];
	vec2_ top_texcoord = vOutTexcoord[1];
	...
	vec3_ left = cross(view_dir, len_dir);
	vec3_ up = cross(left, len_dir);
