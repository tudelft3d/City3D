...
in  vec2_ gOutTexcoord;
in float gOutClipped;

out vec4 outputF;


vec3_ ComputeLight(vec3_ N, vec3_ L, vec3_ V, vec3_ amb, vec3_ spec, float sh, vec3_ color) {
	float df = max(dot(N, L), 0);

	float sf = 0.0;	// specular factor
	if (df > 0.0) {	// if the vertex is lit compute the specular color
		vec3_ half_vector = normalize(L + V);	// compute the half vector
...
void main()
{
	...
	vec3_ view_dir = vec3_(0, 0, 1);	// this is view direction for orthographic mode in camera space
	if (perspective) {
		view_dir = normalize(vec3_(0) - DataIn.point);	// camera pos is (0, 0, 0) in camera space
	}

	// basis = local coordinate system of cone 
	mat3 basis = mat3(DataIn.U, DataIn.V, DataIn.axis);
	vec3_ diff = DataIn.point - DataIn.base;
	vec3_ P = diff * basis;
	...
	vec3_ D = vec3_(dot(DataIn.U, view_dir), dot(DataIn.V, view_dir), dot(DataIn.axis, view_dir));
	...
	vec3_ new_point = DataIn.point + dist * view_dir;

	// now compute the normal of the new point (for computing the lighting).
	vec3_ tangent = cross(new_point - DataIn.base, new_point - DataIn.end);
	vec3_ normal = cross(new_point - DataIn.end, tangent);
	...
	vec3_ projection = DataIn.end + view_dir * tt;
	...
	vec2_ clipZW = new_point.z * PROJ[2].zw + PROJ[3].zw;
	...
	vec3_ color = texture(textureID, gOutTexcoord).rgb;
	vec3_ final_color = ComputeLight(normal, light_dir, view_dir, ambient, specular, shininess, color);
