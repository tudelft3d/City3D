...
void main()
{
    ...
    vec3_ color = texture(textureID, FragmentIn.texcoord).rgb;
    if (!lighting) {
    ...
    vec3_ view_dir = normalize(wCamPos - FragmentIn.point);// compute view direction and normalize it
    vec3_ normal = FragmentIn.normal;
