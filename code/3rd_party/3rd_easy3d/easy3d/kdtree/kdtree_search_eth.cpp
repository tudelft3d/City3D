...
namespace easy3d {
    ...
    int KdTreeSearch_ETH::find_closest_point(const vec3_& p) const {
    ...
    int KdTreeSearch_ETH::find_closest_point(const vec3_& p, float& squared_distance) const {
        kdtree::Vector3D v3d( p.x, p.y, p.z );
        get_tree(tree_)->setNOfNeighbours( 1 );
        get_tree(tree_)->queryPosition( v3d );
    ...
    void KdTreeSearch_ETH::find_closest_k_points(
            const vec3_& p, int k, std::vector<int>& neighbors
        )  const {
            kdtree::Vector3D v3d( p.x, p.y, p.z );
    ...
    void KdTreeSearch_ETH::find_closest_k_points(
            const vec3_& p, int k, std::vector<int>& neighbors, std::vector<float>& squared_distances
        )  const {
            kdtree::Vector3D v3d( p.x, p.y, p.z );
            get_tree(tree_)->setNOfNeighbours( k );
            get_tree(tree_)->queryPosition( v3d );
    ...
    void KdTreeSearch_ETH::find_points_in_range(
            const vec3_& p, float squared_radius, std::vector<int>& neighbors
        )  const {
            kdtree::Vector3D v3d( p.x, p.y, p.z );
    ...
    void KdTreeSearch_ETH::find_points_in_range(
            const vec3_& p, float squared_radius, std::vector<int>& neighbors, std::vector<float>& squared_distances
        )  const {
            kdtree::Vector3D v3d( p.x, p.y, p.z );
            get_tree(tree_)->queryRange( v3d, squared_radius, true );
    ...
    int KdTreeSearch_ETH::find_points_in_cylinder(
            const vec3_& p1, const vec3_& p2, float radius,
    ...
    int KdTreeSearch_ETH::find_points_in_cone(
            const vec3_& eye, const vec3_& p1, const vec3_& p2, float angle_range,
            std::vector<int>& neighbors, std::vector<float>& squared_distances,
            bool bToLine
        ) const {
