...
namespace easy3d {
    ...
    struct PointSet {
        explicit PointSet(const std::vector<vec3_>* points) : pts(points) {}
        const std::vector<vec3_>*  pts;
    ...
    KdTreeSearch_NanoFLANN::KdTreeSearch_NanoFLANN(const PointCloud *cloud) : KdTreeSearch(cloud) {
        // prepare data
        points_ = const_cast< std::vector<vec3_>* >(&cloud->points());
    ...
    int KdTreeSearch_NanoFLANN::find_closest_point(const vec3_& p, float& squared_distance) const {
        std::size_t index;
    ...
    int KdTreeSearch_NanoFLANN::find_closest_point(const vec3_& p) const {
        float dist = 0;
