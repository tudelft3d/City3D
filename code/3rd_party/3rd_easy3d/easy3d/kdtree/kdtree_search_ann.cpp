...
namespace easy3d {
    ...
    KdTreeSearch_ANN::KdTreeSearch_ANN(const PointCloud *cloud) : KdTreeSearch(cloud) {
        ...
        const std::vector<vec3_>& pts = cloud->points();
    ...
    int KdTreeSearch_ANN::find_closest_point(const vec3_& p) const {
        ANNcoord ann_p[3];
    ...
    int KdTreeSearch_ANN::find_closest_point(const vec3_& p, float& squared_distance) const {
        ANNcoord ann_p[3];
        ann_p[0] = p[0];
        ann_p[1] = p[1];
        ann_p[2] = p[2];
