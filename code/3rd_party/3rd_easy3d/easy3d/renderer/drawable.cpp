...
namespace easy3d {
    ...
    void Drawable::buffer_stats(std::ostream &output) const {
        if (vertex_buffer()) {
            ...
            output << "\t\tvertex buffer:     " << num_vertices_ << " vertices, "
                   << num_vertices_ * sizeof(vec3_) << " bytes" << std::endl;
        }
        if (normal_buffer()) {
        ...
        if (texcoord_buffer()) {
            output << "\t\ttexcoord buffer:   " << num_vertices_ << " texcoords, "
                   << num_vertices_ * sizeof(vec2_) << " bytes" << std::endl;
        }
        if (element_buffer()) {
    ...
    void Drawable::update_vertex_buffer(const std::vector<vec3_> &vertices, bool dynamic) {
        assert(vao_);

        bool success = vao_->create_array_buffer(vertex_buffer_, ShaderProgram::POSITION, vertices.data(),
                                                 vertices.size() * sizeof(vec3_), 3, dynamic);
