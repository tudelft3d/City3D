...
namespace easy3d {
    ...
    void Renderer::set_default_rendering_state(PointCloud *model, PointsDrawable *drawable) {
        ...
        auto colors = model->get_vertex_property<vec3_>("v:color");
        if (colors) {
            drawable->set_property_coloring(State::VERTEX, "v:color");
            return;
        }

        auto texcoord = model->get_vertex_property<vec2_>("v:texcoord");
        if (texcoord) {
    ...
    void Renderer::set_default_rendering_state(SurfaceMesh *model, TrianglesDrawable *drawable) {
        ...
        auto face_colors = model->get_face_property<vec3_>("f:color");
        if (face_colors) {
            drawable->set_property_coloring(State::FACE, "f:color");
            return;
        }

        auto vertex_colors = model->get_vertex_property<vec3_>("v:color");
        if (vertex_colors) {
            drawable->set_property_coloring(State::VERTEX, "v:color");
            return;
        }

        auto halfedge_texcoords = model->get_halfedge_property<vec2_>("h:texcoord");
        if (halfedge_texcoords) {
            drawable->set_texture_coloring(State::HALFEDGE, "h:texcoord");
            return;
        }

        auto vertex_texcoords = model->get_vertex_property<vec2_>("v:texcoord");
        if (vertex_texcoords) {
            drawable->set_texture_coloring(State::VERTEX, "v:texcoord");
            return;
        }
