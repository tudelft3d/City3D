...
namespace easy3d {
    ...
    namespace transform {
        ...
        */
        ...
        vec3_ project(const vec3_& obj, const mat4& mv, const mat4& proj, const int viewport[4], bool lowerleft /* = true*/) {
            mat4 mvp = proj * mv;
            vec3_ vs = mvp * obj * 0.5f + vec3_(0.5f);
        ...
        vec3_ project(const vec3_& obj, const mat4& mvp, const int viewport[4], bool lowerleft /* = true*/) {
            vec3_ vs = mvp * obj * 0.5f + vec3_(0.5f);
            vs.x = vs.x * static_cast<float>(viewport[2]) + static_cast<float>(viewport[0]);
            vs.y = vs.y * static_cast<float>(viewport[3]) + static_cast<float>(viewport[1]);
        ...
        vec3_ unproject(const vec3_& win, const mat4& mv, const mat4& proj, const int viewport[4], bool lowerleft /* = true*/) {
            vec3_ vs(
                win.x,
        ...
        vec3_ unproject(const vec3_& win, const mat4& mvp, const int viewport[4], bool lowerleft /* = true*/) {
            vec3_ vs(
                win.x,
                lowerleft ? win.y : (static_cast<float>(viewport[3]) - 1 - win.y),
                win.z
        ...
        mat4 look_at(const vec3_& eye, const vec3_& center, const vec3_& up) {
            const vec3_ f(normalize(center - eye));
            const vec3_ s(normalize(cross(f, up)));
            const vec3_ u(cross(s, f));
        ...
        mat4 pick_matrix(const vec2_& center, const vec2_& delta, const vec4& viewport) {
