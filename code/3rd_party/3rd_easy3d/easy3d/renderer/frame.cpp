...
namespace easy3d {
	...
	/*! Creates a Frame with a position() and an orientation().
	 ...
	 See the vec3_ and quat documentations for convenient constructors and
	 methods.

	 The Frame is defined in the world coordinate system (its referenceFrame() is \c
	 NULL). It has a \c NULL associated constraint(). */
	Frame::Frame(const vec3_ &position, const quat &orientation)
	...
	/*! Translates the Frame of \p t (defined in the Frame coordinate system).
	  ...
	  can be filtered by the constraint(). Use translate(vec3_&) or
	...
	void Frame::translate(const vec3_ &t) {
		vec3_ tbis = t;
		translate(tbis);
	}

	/*! Same as translate(const vec3_&) but \p t may be modified to satisfy the
	  translation constraint(). Its new value corresponds to the translation that
	  has actually been applied to the Frame. */
	void Frame::translate(vec3_ &t) {
		if (constraint())
	...
	/*! Rotates the Frame by \p q (defined in the Frame coordinate system): R = R*q.
	  ...
	  See also translate(const vec3_&). Emits the modified() signal. */
	...
	void Frame::rotateAroundPoint(quat &rotation, const vec3_ &point) {
		if (constraint())
	...
	/*! Same as rotateAroundPoint(), but with a \c const \p rotation quat.
	  ...
	  constraint(). */
	void Frame::rotateAroundPoint(const quat &rotation, const vec3_ &point) {
		quat rot = rotation;
	...
	void Frame::setPosition(const vec3_ &position) {
		if (referenceFrame())
	...
	void Frame::setPositionAndOrientation(const vec3_ &position,
	...
	void Frame::setTranslationAndRotation(const vec3_ &translation,
		const quat &rotation) {
		t_ = translation;
		q_ = rotation;
        modified.send();
	...
	vec3_ Frame::position() const {
		if (referenceFrame_)
			return inverseCoordinatesOf(vec3_(0.0, 0.0, 0.0));
		else
			return t_;
	}
	...
	void Frame::setTranslationWithConstraint(vec3_ &translation) {
		vec3_ deltaT = translation - this->translation();
		if (constraint())
	...
	void Frame::setTranslationAndRotationWithConstraint(vec3_ &translation,
                                                        quat &rotation) {
		vec3_ deltaT = translation - this->translation();
		quat deltaQ = this->rotation().inverse() * rotation;
	...
	void Frame::setPositionWithConstraint(vec3_ &position) {
		if (referenceFrame())
	...
	void Frame::setPositionAndOrientationWithConstraint(vec3_ &position,
                                                        quat &orientation) {
		if (referenceFrame()) {
			position = referenceFrame()->coordinatesOf(position);
			orientation = referenceFrame()->orientation().inverse() * orientation;
	...
	/*! Returns the Frame coordinates of a point \p src defined in the world
	 ...
	 for an illustration. */
	vec3_ Frame::coordinatesOf(const vec3_ &src) const {
		if (referenceFrame())
	...
	vec3_ Frame::inverseCoordinatesOf(const vec3_ &src) const {
		const Frame *fr = this;
		vec3_ res = src;
        while (fr != nullptr) {
	...
	/*! Returns the Frame coordinates of a point \p src defined in the
	  ...
	  localTransformOf(). */
	vec3_ Frame::localCoordinatesOf(const vec3_ &src) const {
	...
	vec3_ Frame::localInverseCoordinatesOf(const vec3_ &src) const {
	...
	vec3_ Frame::coordinatesOfFrom(const vec3_ &src, const Frame *const from) const {
		if (this == from)
			return src;
	...
	vec3_ Frame::coordinatesOfIn(const vec3_ &src, const Frame *const in) const {
		const Frame *fr = this;
		vec3_ res = src;
	...
	/*! Returns the Frame transform of a vector \p src defined in the world
	 ...
	 See the <a href="../examples/frameTransform.html">frameTransform example</a>
	 for an illustration. */
	vec3_ Frame::transformOf(const vec3_ &src) const {
		if (referenceFrame())
	...
	vec3_ Frame::inverseTransformOf(const vec3_ &src) const {
		const Frame *fr = this;
		vec3_ res = src;
        while (fr != nullptr) {
	...
	/*! Returns the Frame transform of a vector \p src defined in the
	  ...
	  localCoordinatesOf(). */
	vec3_ Frame::localTransformOf(const vec3_ &src) const {
	...
	vec3_ Frame::localInverseTransformOf(const vec3_ &src) const {
	...
	vec3_ Frame::transformOfFrom(const vec3_ &src, const Frame *const from) const {
		if (this == from)
			return src;
		else if (referenceFrame())
	...
	vec3_ Frame::transformOfIn(const vec3_ &src, const Frame *const in) const {
		const Frame *fr = this;
		vec3_ res = src;
        while ((fr != nullptr) && (fr != in)) {
			res = fr->localInverseTransformOf(res);
	...
	void Frame::alignWithFrame(const Frame *const frame, bool move, float threshold) {
		vec3_ directions[2][3];
		for (unsigned short d = 0; d < 3; ++d) {
			vec3_ dir((d == 0) ? 1.0f : 0.0f, (d == 1) ? 1.0f : 0.0f, (d == 2) ? 1.0f : 0.0f);
			if (frame)
		...
		if (std::fabs(coef) >= threshold) {
			vec3_ axis = cross(directions[0][index[0]], directions[1][index[1]]);
	...
	normalized. */
	void Frame::projectOnLine(const vec3_ &origin, const vec3_ &direction) {
		...
		const vec3_& shift = origin - position();
		const Line3& line = Line3::from_point_and_direction(vec3_(0, 0, 0), vec3_(direction));
		const vec3_& proj = line.projection(shift);
