...
namespace easy3d {
    ...
    void Shadow::compute_camera_frustum() {
        ...
        const vec3_& pos = camera_->position();
        const vec3_& at = camera_->position() + camera_->viewDirection();
        const vec3_& up = camera_->upVector();
    ...
    void Shadow::compute_light_frustum() {
		...
		const vec3_& at = camera_->sceneCenter();
        vec3_ dir = -setting::light_position.xyz();
        ...
        const vec3_& up = trans * vec3_(0, 1, 0);
        ...
        if (!virtual_background_) {
            ...
            const std::vector<vec3_>& points = camera_frustum_->vertices();
    ...
    void Shadow::update_virtual_background() {
        vec3_ normal = vec3_(setting::light_position); // the background's normal in VC.
        const mat3& trans = transform::normal_matrix(inverse(camera_->modelViewMatrix()));
        normal = normalize(trans * normal);		  // the background's normal in WC.
        float dist = camera_->sceneRadius() * 0.002f;
        const vec3_& center = light_frustum_->far_center() + normal * dist;

        vec3_ up = light_frustum_->up_vector();
        vec3_ dx = cross(up, normal);
        float size = light_frustum_->far_width();
        dx = normalize(dx) * size;
        vec3_ dy = normalize(up) * size;
        float tc = 1.0f;

        std::vector<vec3_> vertices;
        std::vector<vec2_> texcoords;
        std::vector<unsigned int> indices;
        vec3_ a(center - dx - dy);	vertices.push_back(a);	texcoords.emplace_back(vec2_(-tc, -tc));
        vec3_ b(center + dx - dy);	vertices.push_back(b);	texcoords.emplace_back(vec2_(+tc, -tc));
        vec3_ c(center + dx + dy);	vertices.push_back(c);	texcoords.emplace_back(vec2_(+tc, +tc));
        vec3_ d(center - dx + dy);	vertices.push_back(d);	texcoords.emplace_back(vec2_(-tc, +tc));
        indices.push_back(0); indices.push_back(1); indices.push_back(2); // I will render the quad as two triangles
        indices.push_back(0); indices.push_back(2); indices.push_back(3);

        std::vector<vec3_> normals;
    ...
    void Shadow::draw_light_frustum() {
        vec3_ normal = setting::light_position.xyz(); // the background's normal in VC.
        const mat3& trans = transform::normal_matrix(inverse(camera_->modelViewMatrix()));
        normal = normalize(trans * normal);		  // the background's normal in WC.
        float dist = camera_->sceneRadius() * 0.004f;
        const vec3_& offset = normal * dist;

        std::vector<vec3_> points = light_frustum_->vertices();
        points[4] += offset;
        points[5] += offset;
        points[6] += offset;
        points[7] += offset;
