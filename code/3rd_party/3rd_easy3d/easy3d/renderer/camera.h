...
namespace easy3d {
	...
	public:
		...
		vec3_ upVector() const;
		vec3_ viewDirection() const;
		vec3_ rightVector() const;
        ...
        //     - view direction: transpose(R) * vec3_(0, 0, -1), which is actually the negative Z axis of the frame.
        void set_from_calibration(float fx, float fy, float skew, float cx, float cy,
                                  const mat3& R, const vec3_& t, bool convert = true);
	...
	public:
		void setPosition(const vec3_ &pos) const;
		void setOrientation(const quat &q) const;
		void setOrientation(float theta, float phi) const;
		void setUpVector(const vec3_ &up, bool noMove = true) const;
		void setViewDirection(const vec3_ &direction) const;
		//@}

		/*! @name Positioning tools */
	public:
		void lookAt(const vec3_ &target) const;
		void showEntireScene() const;
		void fitSphere(const vec3_ &center, float radius) const;
		void fitBoundingBox(const vec3_ &min, const vec3_ &max) const;
    ...
    public:
        ...
        void interpolateToLookAt(const vec3_& point);
	...
	public:
		...
		float pixelGLRatio(const vec3_ &position) const;
	...
	public:
		 ...
		 * \see setSceneBoundingBox().
		 */
		vec3_ sceneCenter() const { return sceneCenter_; }
		float distanceToSceneCenter() const;

	public:
		void setSceneRadius(float radius);
		void setSceneCenter(const vec3_ &center);
		void setSceneBoundingBox(const vec3_ &min, const vec3_ &max);
		//@}

	public:
		// The camera will move around the pivot point if set.
		void setPivotPoint(const vec3_ &point);
		vec3_ pivotPoint() const;
	...
	public:
		...
		//@{
	public:
		vec3_ cameraCoordinatesOf(const vec3_ &src) const;
		vec3_ worldCoordinatesOf(const vec3_ &src) const;
		//@}

		/*! @name 2D screen to 3D world coordinate systems conversions */
		//@{
	public:
        vec3_ projectedCoordinatesOf(const vec3_ &src, const Frame *frame = nullptr) const;
        vec3_ unprojectedCoordinatesOf(const vec3_ &src, const Frame *frame = nullptr) const;
		void convertClickToLine(int x, int y, vec3_ &orig, vec3_ &dir) const;
		//@}
