...
namespace easy3d {
	...
	void ManipulatedCameraFrame::action_rotate(int x, int y, int dx, int dy, Camera *const camera, ScreenAxis axis)
	{
        ...
        const vec3_& trans = camera->projectedCoordinatesOf(pivotPoint());
        ...
        switch (axis) {
            case NONE:
                ...
                break;
            case ORTHOGONAL:
                cons.setRotationConstraint(AxisPlaneConstraint::AXIS, vec3_(0, 0, 1));
                setConstraint(&cons);
                break;
            case VERTICAL:
                cons.setRotationConstraint(AxisPlaneConstraint::AXIS, vec3_(0, 1, 0));
                setConstraint(&cons);
                break;
            case HORIZONTAL:
                cons.setRotationConstraint(AxisPlaneConstraint::AXIS, vec3_(1, 0, 0));
                setConstraint(&cons);
                break;
        }
	...
	void ManipulatedCameraFrame::action_translate(int x, int y, int dx, int dy, Camera *const camera, ScreenAxis axis)
	{
        ...
        if (axis == NONE) {    // free translation
            vec3_ trans(-static_cast<float>(dx), static_cast<float>(dy), 0.0f);
			...
			vec3_ trans;
            if (axis == HORIZONTAL)
                trans = vec3_(-static_cast<float>(dx), 0.0f, 0.0f);
            else if (axis == VERTICAL)
                trans = vec3_(0.0f, static_cast<float>(dy), 0.0f);
            else
                return;
	...
	void ManipulatedCameraFrame::action_zoom(int wheel_dy, Camera *const camera)
	{
		...
		if (zoomsOnPivotPoint_) {
			const vec3_& direction = camera->pivotPoint() - position();
            ...
            const vec3_& new_pos = position() + delta * direction;
            const vec3_& offset = camera->pivotPoint() - new_pos;
            ...
            // in the camera space
            const vec3_& target = camera->frame()->coordinatesOf(camera->sceneCenter());
            const float coef = target.z;//std::max<float>(fabs(target.z), 0.2f * sceneRadius);
            const vec3_ trans_eye(0.0f, 0.0f, coef * delta);

            // translation in the world space
            const vec3_ offset = inverseTransformOf(trans_eye);
...
#if 0 //  we allow to zoom out (to avoid lock)
            const float sceneRadius = camera->sceneRadius();
            const vec3_& new_target = frame.coordinatesOf(camera->sceneCenter());
            // don't get too close to and don't get too far away from the pivot point.
	...
	void ManipulatedCameraFrame::action_turn(float angle_radian, Camera *const camera) {
        ...
        const quat rot(vec3_(0.0, 1.0, 0.0), angle_radian);
		rotate(rot);
        modified.send();
	}
