...
namespace easy3d {
	...
	*/
	...
	void Camera::computeModelViewMatrix()
	{
		...
		const vec3_ t = q.inverse_rotate(position());
	...
	void Camera::setSceneBoundingBox(const vec3_ &min, const vec3_ &max) {
	...
	void Camera::setSceneCenter(const vec3_ &center) {
		sceneCenter_ = center;
	 ...
	 * system). */
	void Camera::setPivotPoint(const vec3_ &point) {
	...
	float Camera::pixelGLRatio(const vec3_ &position) const {
		switch (type()) {
		case Camera::PERSPECTIVE:
    ...
    void Camera::interpolateToLookAt(const vec3_ &p) {
        const float coef = 0.1f;
        const vec3_& pos = coef * frame()->position() + (1.0f - coef) * p;
	...
	/*! Sets the Camera orientation(), so that it looks at point \p target (defined
	 ...
	 fitBoundingBox(). */
	void Camera::lookAt(const vec3_ &target) const {
	...
	void Camera::fitSphere(const vec3_ &center, float radius) const {
		float distance = 0.0f;
		switch (type()) {
		case Camera::PERSPECTIVE: {
		...
		vec3_ newPos(center - distance * viewDirection());
	...
	void Camera::fitBoundingBox(const vec3_ &min, const vec3_ &max) const {
        const float radius = distance(min, max) * 0.5f;
        const vec3_& center = 0.5f * (min + max);
	...
	void Camera::fitScreenRegion(int xmin, int ymin, int xmax, int ymax) const {
		...
		const vec3_& newCenter = orig + distToPlane / dot(dir, vd) * dir;

		convertClickToLine(xmin, cy, orig, dir);
		const vec3_& pointX = orig + distToPlane / dot(dir, vd) * dir;

		convertClickToLine(cx, ymin, orig, dir);
		const vec3_& pointY = orig + distToPlane / dot(dir, vd) * dir;
		...
		vec3_ newPos(newCenter - distance * vd);
		frame()->setPositionWithConstraint(newPos);
	}
	...
	void Camera::setUpVector(const vec3_ &up, bool noMove) const {
		quat q(vec3_(0.0, 1.0, 0.0), frame()->transformOf(up));
	...
	void Camera::setOrientation(float theta, float phi) const {
		vec3_ axis(0.0f, 1.0f, 0.0f);
		const quat rot1(axis, theta);
		axis = vec3_(-std::cos(theta), 0.0f, std::sin(theta));
	...
	/*! Rotates the Camera so that its viewDirection() is \p direction (defined in
	 ...
	 setUpVector(). */
	void Camera::setViewDirection(const vec3_ &direction) const {
		if (direction.length2() < 1E-10)
			return;

		vec3_ xAxis = cross(direction, upVector());
		if (xAxis.length2() < 1E-10)
		{
			...
			xAxis = frame()->inverseTransformOf(vec3_(1.0, 0.0, 0.0));
		}
	...
	ahead. */
	vec3_ Camera::position() const { return frame()->position(); }
	...
	frame()->inverseTransformOf(vec3_(0.0, 1.0, 0.0)) ). */
	vec3_ Camera::upVector() const {
		return frame()->inverseTransformOf(vec3_(0.0, 1.0, 0.0));
	}
	...
	frame()->inverseTransformOf(vec3_(0.0, 0.0, -1.0)) ). */
	vec3_ Camera::viewDirection() const {
		return frame()->inverseTransformOf(vec3_(0.0, 0.0, -1.0));
	}
	...
	Simply returns frame()->inverseTransformOf(vec3_(1.0, 0.0, 0.0)). */
	vec3_ Camera::rightVector() const {
		return frame()->inverseTransformOf(vec3_(1.0, 0.0, 0.0));
	}
	 ...
	 * system. */
	void Camera::setPosition(const vec3_ &pos) const { frame()->setPosition(pos); }
	...
	*/
	vec3_ Camera::cameraCoordinatesOf(const vec3_ &src) const {
	...
	vec3_ Camera::worldCoordinatesOf(const vec3_ &src) const {
	...
	vec3_ Camera::pivotPoint() const { return frame()->pivotPoint(); }
	...
	void Camera::set_from_model_view_matrix(const mat4& mv) const
	{
		...
		setOrientation(q);
		setPosition(-q.rotate(vec3_(mv(0, 3), mv(1, 3), mv(2, 3))));
	}


    void Camera::set_from_calibration(float fx, float fy, float skew, float cx, float cy,
                                      const mat3 &R, const vec3_ &t, bool convert)
    {
    ...
    void Camera::set_from_calibration(const mat34 &proj) {
        ...
        vec3_ line_0 = vec3_(proj.row(0));	line_0.normalize();
        vec3_ line_1 = vec3_(proj.row(1));	line_1.normalize();
        vec3_ line_2 = vec3_(proj.row(2));	line_2.normalize();
        ...
        vec3_ intersection = cross(line_1, column_0); // vector of intersection line between y_screen=0 and x_camera=0 plane.
	...
	/*! Returns the screen projected coordinates of a 3D point \p src defined in the \p
	 ...
	 The x and y coordinates of the returned vec3_ are expressed in pixel, (0,0) being
	 ...
	 Robert W. Kuhn) : \code vec3_ project(vec3_ point)
	 {
	  int    Viewport[4];
	  ...
	  return vec3_(vs[0], Viewport[3]-vs[1], vs[2]);
	  }
	 \endcode
	 */
	vec3_ Camera::projectedCoordinatesOf(const vec3_ &src, const Frame *frame) const {
		vec3_ tmp = src;
		if (frame)
		...
		vec3_ vs = mvp * tmp * 0.5f + vec3_(0.5f);
		...
		return vec3_(vs.x, static_cast<float>(viewport[3]) - 1.0f - vs.y, vs.z);
	}
	...
	vec3_ Camera::unprojectedCoordinatesOf(const vec3_ &src, const Frame *frame) const {
		const mat4& mvp = modelViewProjectionMatrix();

		const int viewport[] = { 0, 0, screenWidth_, screenHeight_ };
		vec3_ vs(src.x, static_cast<float>(viewport[3]) - 1.0f - src.y, src.z);
	...
	/*! Gives the coefficients of a 3D half-line passing through the Camera eye and
	 ...
	 illustration. */
	void Camera::convertClickToLine(int x, int y, vec3_ &orig, vec3_ &dir) const {
		switch (type()) {
		case Camera::PERSPECTIVE:
			orig = position();
                dir = vec3_(
	...
	void Camera::getFrustumPlanesCoefficients(float coef[6][4]) const
	{
		...
		const vec3_ pos = position();
		const vec3_ viewDir = viewDirection();
		const vec3_ up = upVector();
		const vec3_ right = rightVector();
		const float posViewDir = dot(pos, viewDir);

		static vec3_ normal[6];
		static float dist[6];
