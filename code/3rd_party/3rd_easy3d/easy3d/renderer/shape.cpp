...
namespace easy3d {
    ...
    namespace shape {
        ...
        void draw_quad_wire(const Rect &rect, const vec4 &color, int width, int height, float depth) {
            ...
            vao.create_array_buffer(vertex_buffer, ShaderProgram::POSITION, points.data(), points.size() * sizeof(vec2_),
        ...
        void draw_quad_filled(const Rect &rect, unsigned int texture, int width, int height, float depth) {
            ...
            static const std::vector<vec2_> texcoords = {
                    vec2_(0, 0),
                    vec2_(1.0, 0),
                    vec2_(1.0, 1.0),
                    vec2_(0, 1.0),
            ...
            vao.create_array_buffer(texcoord_buffer, ShaderProgram::TEXCOORD, texcoords.data(),
                                    texcoords.size() * sizeof(vec2_), 2, true);    easy3d_debug_log_gl_error
            vao.create_element_buffer(element_buffer, indices.data(), indices.size() * sizeof(unsigned int),
                                      true);    easy3d_debug_log_gl_error
        ...
        void draw_full_screen_quad(unsigned int texture, float depth) {
            ...
            static const std::vector<vec2_> points = {
                    vec2_(-1.0f, -1.0f),
                    vec2_(1.0f, -1.0f),
                    vec2_(1.0f, 1.0f),
                    vec2_(-1.0f, 1.0f)
                       ...
                       int vpw, int vph, float depth)
       {
        ...
        void draw_polygon_wire(const Polygon2 &polygon, const vec4 &color, int width, int height, float depth) {
            ...
            std::vector<vec2_> points(polygon.size());
        ...
        void draw_polygon_filled(const Polygon2 &polygon, const vec4 &color, int width, int height, float depth) {
            ...
            // draw the face
            Tessellator tess;
            tess.begin_polygon(vec3_(0, 0, 1));
            tess.begin_contour();
            for (std::size_t i = 0; i < polygon.size(); ++i) {
                const auto &p = polygon[i];
                // to use the screen space shaders, I need to convert the point coordinates into the NDC space.
                ...
                tess.add_vertex(vec3_(x, y, 0));
            }
            tess.end_contour();
            tess.end_polygon();
            const auto &vts = tess.vertices();
            std::vector<vec2_> points(vts.size());
            for (std::size_t i = 0; i < vts.size(); ++i)
                points[i] = vec2_(vts[i]->data());
        ...
        void draw_sphere_big_circles(LinesDrawable *drawable, const mat4 &mvp, const mat4 &m, bool axes) {
            ...
            if (drawable->vertex_buffer() == 0) {
                std::vector<vec3_> points, colors;
                std::vector<unsigned int> indices;

                std::vector<vec3_> points_xoy;   // xoy
                create_circle(50, points_xoy, indices);

                for (auto &p : points_xoy) {
                    points.push_back(p);
                    colors.emplace_back(vec3_(0, 0, 1));
                }
                colors.resize(points_xoy.size(), vec3_(0, 0, 1));

                auto rot_x = mat4::rotation(vec3_(1, 0, 0), static_cast<float>(M_PI * 0.5f));
                for (std::size_t i = 0; i < points_xoy.size(); ++i) {
                    points.push_back(rot_x * points_xoy[i]);
                    colors.emplace_back(vec3_(0, 1, 0));
                ...
                auto rot_y = mat4::rotation(vec3_(0, 1, 0), static_cast<float>(M_PI * 0.5f));
                for (std::size_t i = 0; i < points_xoy.size(); ++i) {
                    points.push_back(rot_y * points_xoy[i]);
                    colors.emplace_back(vec3_(1, 0, 0));
                ...
                if (axes) {
                    // x axis
                    points.emplace_back(vec3_(-1, 0, 0));
                    points.emplace_back(vec3_(1, 0, 0));
                    colors.emplace_back(vec3_(1, 0, 0));
                    colors.emplace_back(vec3_(1, 0, 0));
                    ...
                    // y axis
                    points.emplace_back(vec3_(0, -1, 0));
                    points.emplace_back(vec3_(0, 1, 0));
                    colors.emplace_back(vec3_(0, 1, 0));
                    colors.emplace_back(vec3_(0, 1, 0));
                    indices.push_back(static_cast<unsigned int>(points.size() - 2));
                    ...
                    // z axis
                    points.emplace_back(vec3_(0, 0, -1));
                    points.emplace_back(vec3_(0, 0, 1));
                    colors.emplace_back(vec3_(0, 0, 1));
                    colors.emplace_back(vec3_(0, 0, 1));
                    indices.push_back(static_cast<unsigned int>(points.size() - 2));
        ...
        void create_grid(int x_steps, int y_steps, std::vector<vec3_> &points, float depth, float scale) {
            // Horizontal lines
            float x = scale * 0.5f * static_cast<float>(x_steps - 1);
            float y = -scale * 0.5f * static_cast<float>(y_steps - 1);
            for (int i = 0; i < y_steps; i++) {
                points.emplace_back(vec3_(-x, y, depth));
                points.emplace_back(vec3_(x, y, depth));
                y += scale;
            }
        ...
        void create_box(std::vector<vec3_> &points, std::vector<vec3_> &colors, bool abstracted) {
            points.clear();
            colors.clear();

            const float min_coord = -0.5f;
            const float max_coord = 0.5f;
            vec3_ red(1, 0, 0), green(0, 1, 0), blue(0, 0, 1);

            std::vector<vec3_> vts = {
                    vec3_(min_coord, min_coord, min_coord), // 0
                    vec3_(max_coord, min_coord, min_coord), // 1
                    vec3_(max_coord, max_coord, min_coord), // 2
                    vec3_(min_coord, max_coord, min_coord), // 3
                    vec3_(min_coord, min_coord, max_coord), // 4
                    vec3_(max_coord, min_coord, max_coord), // 5
                    vec3_(max_coord, max_coord, max_coord), // 6
                    vec3_(min_coord, max_coord, max_coord)  // 7
            };
        ...
        void create_circle(int slices, std::vector<vec3_> &points, std::vector<unsigned int> &indices) {
            points.clear();
            indices.clear();
            ...
            for (int i = 0; i < slices; ++i) {
                ...
                points.emplace_back(vec3_(x, y, 0.f));
        ...
        void create_sphere(
                const vec3_ &center, double radius, int slices, int stacks, const vec3_ &color,
                std::vector<vec3_> &points, std::vector<vec3_> &normals, std::vector<vec3_> &colors) {
            create_checker_sphere(center, radius, slices, stacks, 1, color, color, points, normals, colors);
        }


        void create_checker_sphere(
                const vec3_ &center, double radius, int slices, int stacks, int checker_size,
                const vec3_ &color1, const vec3_ &color2,
                std::vector<vec3_> &points, std::vector<vec3_> &normals, std::vector<vec3_> &colors) {
            for (int u = 0; u < slices; u += 1) {
                ...
                for (int v = 0; v < stacks; v++) {
                    ...
                    std::vector<vec3_> fan;
                    fan.emplace_back(vec3_(static_cast<float>(x11), static_cast<float>(y11), static_cast<float>(z11)));
                    if (v != 0)
                        fan.emplace_back(vec3_(static_cast<float>(x21), static_cast<float>(y21), static_cast<float>(z21)));
                    fan.emplace_back(vec3_(static_cast<float>(x22), static_cast<float>(y22), static_cast<float>(z22)));
                    if (v != stacks - 1)
                        fan.emplace_back(vec3_(static_cast<float>(x12), static_cast<float>(y12), static_cast<float>(z12)));

                    int toggle = ((u / checker_size) ^ (v / checker_size)) & 1;
                    vec3_ color = toggle ? color1 : color2;
        ...
        void create_cylinder(double radius, int slices, const vec3_ &s, const vec3_ &t, const vec3_ &color,
                             std::vector<vec3_> &points, std::vector<vec3_> &normals, std::vector<vec3_> &colors) {
            // find a vector perpendicular to the direction
            const vec3_ offset = t - s;
            const vec3_ axis = normalize(offset);
            vec3_ perp = geom::orthogonal(axis);
            perp.normalize();
            const vec3_ p = s + perp * radius;
            ...
            for (int i = 0; i < slices; ++i) {
                ...
                const vec3_ a = s + mat4::rotation(axis, static_cast<float>(angle_a)) * (p - s);
                const vec3_ b = s + mat4::rotation(axis, static_cast<float>(angle_b)) * (p - s);
                const vec3_ c = a + offset;
                const vec3_ d = b + offset;
                const vec3_ na = normalize(a - s);
                const vec3_ nb = normalize(b - s);
                const vec3_ nc = normalize(c - t);
                const vec3_ nd = normalize(d - t);
                // triangle 1
                points.push_back(a);
        ...
        void create_cone(double radius, int slices, const vec3_ &s, const vec3_ &t, const vec3_ &color,
                         std::vector<vec3_> &points, std::vector<vec3_> &normals, std::vector<vec3_> &colors) {
            // find a vector perpendicular to the direction
            const vec3_ offset = t - s;
            const vec3_ axis = normalize(offset);
            ...
            for (int i = 0; i < slices; ++i) {
                ...
                vec3_ dir = a - t;
                dir.normalize();
                vec3_ na = t + dir * dot(s - t, dir) - s;
                na.normalize();

                // nb
                dir = b - t;
                dir.normalize();
                vec3_ nb = t + dir * dot(s - t, dir) - s;
                nb.normalize();

                // nc
                vec3_ nc = cross(a - t, b - t);
                nc.normalize();
        ...
        void create_torus(double major_radius, double minor_radius, int major_slices, int minor_slices,
                          std::vector<vec3_> &points, std::vector<vec3_> &normals) {
            bool flip = false;
            const double twopi = 2.0 * M_PI;
            for (int i = 0; i < minor_slices; i++) {
                // QUAD STRIPs;
                std::vector<vec3_> tmp_pts, tmp_nms;
                for (int j = 0; j <= major_slices; j++) {
                    for (int k = 1; k >= 0; k--) {
                        ...
                        vec3_ n(static_cast<float>(nx), static_cast<float>(ny), static_cast<float>(nz));
                        n.normalize();

                        tmp_pts.emplace_back(vec3_(static_cast<float>(x), static_cast<float>(y), static_cast<float>(z)));
                        tmp_nms.push_back(n);

                        if (tmp_pts.size() <= 3) {
                            points.emplace_back(vec3_(static_cast<float>(x), static_cast<float>(y), static_cast<float>(z)));
                            normals.push_back(n);
                        } else {
                            points.emplace_back(vec3_(static_cast<float>(x), static_cast<float>(y), static_cast<float>(z)));
                            normals.push_back(n);
        ...
        void create_camera(std::vector<vec3_> &points, float width, float fov, float hw_ratio) {
            ...
            // Frustum outline
            //--------------
            const vec3_ c(0.0f, 0.0f, 0.0f);
            const vec3_ p0(-halfWidth, -halfHeight, -dist);
            const vec3_ p1(halfWidth, -halfHeight, -dist);
            const vec3_ p2(halfWidth, halfHeight, -dist);
            const vec3_ p3(-halfWidth, halfHeight, -dist);
            points.push_back(p0);
            points.push_back(p1);
            points.push_back(p1);
            points.push_back(p2);
            ...
            // Base - QUAD
            const vec3_ r0(-baseHalfWidth, halfHeight, -dist);
            const vec3_ r1(baseHalfWidth, halfHeight, -dist);
            const vec3_ r2(baseHalfWidth, baseHeight, -dist);
            const vec3_ r3(-baseHalfWidth, baseHeight, -dist);
            points.push_back(r0);
            points.push_back(r1);
            points.push_back(r1);
            points.push_back(r2);
            ...
            // Arrow - TRIANGLE
            const vec3_ a0(0.0f, arrowHeight, -dist);
            const vec3_ a1(-arrowHalfWidth, baseHeight, -dist);
            const vec3_ a2(arrowHalfWidth, baseHeight, -dist);
            points.push_back(a0);
            points.push_back(a1);
            points.push_back(a1);
            points.push_back(a2);
        ...
        void create_camera(std::vector<vec3_> &points, std::vector<unsigned int> &indices, float width, float fov,
                           float hw_ratio) {
            const float halfWidth = width * 0.5f;
            const float halfHeight = halfWidth * hw_ratio;
            const auto dist = static_cast<float>(halfHeight / std::tan(fov * 0.5));
