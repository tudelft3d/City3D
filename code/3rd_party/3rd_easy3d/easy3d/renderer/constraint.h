...
namespace easy3d {
	...
	/*! \brief An interface class for Frame constraints.
	  ...
	  Constraint: \code Frame::translate(vec3_& T)
	  {
			if (constraint())
	  ...
	  {
	  public:
			virtual void constrainTranslation(vec3_& t, Frame * const fr)
			  {
					// Express t in the world coordinate system.
					const vec3_ tWorld = fr->inverseTransformOf(t);
	  ...
	  myConstraint::constrainTranslation(vec3_& v, Frame* const fr)
	  {
	...
	public:
		...
		needed. */
		virtual void constrainTranslation(vec3_ &translation, Frame *const frame) {
			(void)translation;
			(void)frame;
		}
	...
	public:
        ...
        void constrainTranslation(vec3_ &translation, Frame *const frame) override {
			(void)(translation);
			(void)(frame);
		}

		void setTranslationConstraint(Type type, const vec3_ &direction);
		...
		void setTranslationConstraintDirection(const vec3_ &direction);
		...
		vec3_ translationConstraintDirection() const {
		...
		void setRotationConstraint(Type type, const vec3_ &direction);
		void setRotationConstraintType(Type type);
		void setRotationConstraintDirection(const vec3_ &direction);
		...
		vec3_ rotationConstraintDirection() const { return rotationConstraintDir_; };
		//@}
	...
	private:
		...
		vec3_ translationConstraintDir_;
		vec3_ rotationConstraintDir_;
	};
