...
namespace easy3d {
    ...
    void KeyFrameInterpolator::set_keyframe_position(std::size_t index, const vec3_& pos) {
        stop_interpolation();
    ...
    const vec3_& KeyFrameInterpolator::keyframe_position(std::size_t index) const {
    ...
    const std::vector<Frame>& KeyFrameInterpolator::interpolate() {
        if (pathIsValid_ || keyframes_.empty()) // already fitted or no keyframe
            ...
            for (int i=0; i<num_frames; ++i) {
                ...
                const vec3_ pos = (1.0f - w) * keyframes_[0].position() + w * keyframes_[1].position();
        ...
        if (interpolation_method_ == INTERPOLATION) {
            ...
            std::vector<vec3_> positions(keyframes_.size());
            ...
            // spine interpolation
            typedef SplineCurveInterpolation<vec3_> PosFitter;
            ...
            for (int i = 0; i < num_frames; ++i) {
                ...
                const vec3_ pos = pos_fitter.eval_f(u);
            ...
            typedef SplineCurveFitting <vec3_> PosFitter;
