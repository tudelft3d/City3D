...
namespace easy3d {
    ...
    Frustum::Frustum(ProjectionType type)
        : type_(type)
    {
        orient(vec3_(0.0f, 0.0f, 1.0f), vec3_(0.0f, 0.0f, 0.0f), vec3_(0.0f, 1.0f, 0.0f));
        if (type == ORTHO)
    ...
    void Frustum::orient(const vec3_ &pos, const vec3_& at, const vec3_& up)
    {
        pos_ = pos;
        at_ = at;
        up_ = up;
    ...
    vec3_ Frustum::near_center() const {
        vec3_ view_dir = at_ - pos_;
        view_dir = normalize(view_dir);
        vec3_ nc = pos_ + view_dir * near_;	// near center
        return nc;
    }


    vec3_ Frustum::far_center() const {
        vec3_ view_dir = at_ - pos_;
        view_dir = normalize(view_dir);
        vec3_ fc = pos_ + view_dir * far_;	// far center
        return fc;
    }


    // the up/right vector in world space
    vec3_ Frustum::up_vector() const {
        vec3_ view_dir = at_ - pos_; view_dir = normalize(view_dir);
        vec3_ right = cross(view_dir, up_); right = normalize(right);
        return right;
    }


    vec3_ Frustum::right_vector() const {
        vec3_ view_dir = at_ - pos_; view_dir = normalize(view_dir);
        vec3_ right = cross(view_dir, up_); right = normalize(right);
        vec3_ up = cross(right, view_dir); up = normalize(up);
        return up;
    }
    ...
    std::vector<vec3_> Frustum::vertices() const
    {
    #ifdef compute_in_local_system
        vec3_ view_dir(0, 0, -1);
        vec3_ right(1, 0, 0);
        vec3_ up(0, 1, 0);
        vec3_ nc = view_dir * near_;	// near center
        vec3_ fc = view_dir * far_;	// far center
    #else
        vec3_ view_dir = at_ - pos_; view_dir = normalize(view_dir);
        vec3_ right = cross(view_dir, up_); right = normalize(right);
        vec3_ up = cross(right, view_dir); up = normalize(up);
