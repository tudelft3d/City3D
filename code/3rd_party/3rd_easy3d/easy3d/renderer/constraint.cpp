...
namespace easy3d {
	...
	static void project_on_axis(vec3_& v, const vec3_& direction) {
		Line3 line = Line3::from_point_and_direction(vec3_(0, 0, 0), vec3_(direction));
		v = line.projection(v);
	}
	...
	static void project_on_plane(vec3_& v, const vec3_& normal) {
		Plane3 plane(vec3_(0, 0, 0), normal);
    ...
    void AxisPlaneConstraint::setTranslationConstraint(Type type, const vec3_ &direction) {
	 ...
	 * direction is expressed depends on your class implementation. */
	void AxisPlaneConstraint::setRotationConstraintDirection(const vec3_ &direction) {
	...
	void LocalConstraint::constrainTranslation(vec3_ &translation,
                                               Frame *const frame) {
		vec3_ proj;
	...
	void LocalConstraint::constrainRotation(quat &rotation, Frame *const) {
		...
		case AxisPlaneConstraint::PLANE:
			break;
		case AxisPlaneConstraint::AXIS: {
			vec3_ axis = rotationConstraintDirection();
			vec3_ q = vec3_(rotation[0], rotation[1], rotation[2]);
	...
	void WorldConstraint::constrainTranslation(vec3_ &translation,
                                               Frame *const frame) {
		vec3_ proj;
		switch (translationConstraintType()) {
		case AxisPlaneConstraint::FREE:
	...
	void WorldConstraint::constrainRotation(quat &rotation,
		...
		case AxisPlaneConstraint::PLANE:
			break;
		case AxisPlaneConstraint::AXIS: {
			vec3_ q(rotation[0], rotation[1], rotation[2]);
			vec3_ axis = frame->transformOf(rotationConstraintDirection());
			project_on_axis(q, axis);
			rotation = quat(q, 2.0f * std::acos(rotation[3]));
			break;
		}
	...
	void CameraConstraint::constrainTranslation(vec3_ &translation,
                                                Frame *const frame) {
		vec3_ proj;
		switch (translationConstraintType()) {
		case AxisPlaneConstraint::FREE:
