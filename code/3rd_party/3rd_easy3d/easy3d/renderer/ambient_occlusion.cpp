...
namespace easy3d {
    ...
    void AmbientOcclusion::generate_noise(int width, int height) {
        ...
        for (unsigned int i = 0; i < KERNEL_SIZE; ++i) {
            vec3_ sample(randomFloats(generator) * 2.0f - 1.0f, randomFloats(generator) * 2.0f - 1.0f, randomFloats(generator));
        ...
        std::vector<vec3_> ssaoNoise;
        for (int i = 0; i < width * height; i++) {
            //vec3_ noise(randomFloats(generator) * 2.0 - 1.0, randomFloats(generator) * 2.0 - 1.0, 0.0f);//ssao
            vec3_ noise(randomFloats(generator) * 2.0f - 1.0f, randomFloats(generator) * 2.0f - 1.0f, randomFloats(generator)); // rotate around z-axis (in tangent space)
