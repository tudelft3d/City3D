    ...
    namespace buffer {
        ...
        namespace internal {
            ...
            update_scalar_on_vertices(MODEL *model, PointsDrawable *drawable, typename MODEL::template VertexProperty<FT> prop) {
                ...
                auto points = model->template get_vertex_property<vec3_>("v:point");

                std::vector<vec2_> d_texcoords;
                d_texcoords.reserve(model->n_vertices());
                for (auto v : model->vertices()) {
                    float coord = (prop[v] - min_value) / (max_value - min_value);
                    d_texcoords.emplace_back(vec2_(coord, 0.5f));
                }
                drawable->update_vertex_buffer(points.vector());
                drawable->update_texcoord_buffer(d_texcoords);

                auto normals = model->template get_vertex_property<vec3_>("v:normal");
                if (normals)
            ...
            update_scalar_on_faces(SurfaceMesh *model, TrianglesDrawable *drawable, SurfaceMesh::FaceProperty<FT> prop) {
                ...
                if (model->is_triangle_mesh()) {
                    auto points = model->get_vertex_property<vec3_>("v:point");
                    model->update_vertex_normals();
                    auto normals = model->get_vertex_property<vec3_>("v:normal");
                    ...
                    std::vector<vec3_> d_points, d_normals;
                    std::vector<vec2_> d_texcoords;
                    ...
                    for (auto face : model->faces()) {
                        ...
                        for (auto h : model->halfedges(face)) {
                            ...
                            vertex.append(vec2_(coord, 0.5f));
            ...
            inline void
            update_colors_on_vertices(PolyMesh *model, TrianglesDrawable *drawable, PolyMesh::VertexProperty<vec3_> colors, bool border) {
                assert(model);
                assert(drawable);
                assert(colors);
            ...
            inline void
            update_colors_on_faces(PolyMesh *model, TrianglesDrawable *drawable, PolyMesh::FaceProperty<vec3_> colors, bool border) {
                assert(model);
                assert(drawable);
                assert(colors);
            ...
            inline void
            update_texcoords_on_vertices(PolyMesh *model, TrianglesDrawable *drawable, PolyMesh::VertexProperty<vec2_> vtexcoords, bool border) {
                assert(model);
                assert(drawable);
                assert(vtexcoords);
            ...
            void update_colors_on_vertices(Mesh *model, PointsDrawable *drawable, typename Mesh::template VertexProperty<vec3_> prop) {
                assert(model);
                assert(drawable);
                assert(prop);
            ...
            void update_texcoords_on_vertices(MODEL *model, PointsDrawable *drawable, typename MODEL::template VertexProperty<vec2_> prop) {
                assert(model);
                assert(drawable);
                assert(prop);
...
#if HANDLE_HOLES
                    auto prop_holes = model->get_face_property< std::vector< std::vector<vec3_> > >("f:holes");
#endif
            ...
            void update_colors_on_faces(SurfaceMesh *model, TrianglesDrawable *drawable,
                        SurfaceMesh::FaceProperty<vec3_> fcolor) {
                assert(model);
                assert(drawable);
                assert(fcolor);
                ...
                if (model->is_triangle_mesh()) {
                    ...
                    std::vector<vec3_> d_points, d_normals, d_colors;
                    d_points.reserve(model->n_faces() * 3);
                    d_normals.reserve(model->n_faces() * 3);
                    d_colors.reserve(model->n_faces() * 3);

                    for (auto f : model->faces()) {
                        const vec3_ &color = fcolor[f];
                        for (auto v : model->vertices(f)) {
            ...
            void update_colors_on_vertices(SurfaceMesh *model, TrianglesDrawable *drawable, SurfaceMesh::VertexProperty<vec3_> vcolor) {
                assert(model);
                assert(drawable);
                assert(vcolor);
            ...
            void update_texcoords_on_vertices(SurfaceMesh *model, TrianglesDrawable *drawable,
                        SurfaceMesh::VertexProperty<vec2_> vtexcoords) {
                assert(model);
                assert(drawable);
                assert(vtexcoords);
            ...
            void update_texcoords_on_halfedges(SurfaceMesh *model, TrianglesDrawable *drawable,
                        SurfaceMesh::HalfedgeProperty<vec2_> htexcoords) {
                assert(model);
                assert(drawable);
                assert(htexcoords);
            ...
            template <typename MODEL>
            void update_colors_on_edges(MODEL *model, LinesDrawable *drawable, typename MODEL::template EdgeProperty<vec3_> prop) {
                assert(model);
                assert(drawable);
                assert(prop);
                ...
                auto points = model->template get_vertex_property<vec3_>("v:point");
                std::vector<vec3_> d_points, d_colors;
                d_points.reserve(model->n_edges() * 2);
            ...
            template <typename MODEL>
            void
            update_colors_on_vertices(MODEL *model, LinesDrawable *drawable, typename MODEL::template VertexProperty<vec3_> prop) {
                assert(model);
                assert(drawable);
                assert(prop);
            ...
            template <typename MODEL>
            void
            update_texcoords_on_vertices(MODEL *model, LinesDrawable *drawable, typename MODEL::template VertexProperty<vec2_> prop) {
                assert(model);
                assert(drawable);
                assert(prop);
            ...
            template<typename MODEL>
            void update_texcoords_on_edges(MODEL *model, LinesDrawable *drawable, typename MODEL::template EdgeProperty<vec2_> prop) {
                assert(model);
                assert(drawable);
                assert(prop);
            ...
            void update_mesh_borders(SurfaceMesh *model, LinesDrawable *drawable) {
                ...
                auto prop = model->get_vertex_property<vec3_>("v:point");
                std::vector<vec3_> points;
            ...
            void update_colors_on_vertices(MODEL *model, PointsDrawable *drawable, const std::string& name) {
                auto colors = model->template get_vertex_property<vec3_>(name);
                if (colors)
            ...
            void update_colors_on_edges(MODEL *model, LinesDrawable *drawable, const std::string& name) {
                auto colors = model->template get_edge_property<vec3_>(name);
                if (colors)
            ...
            void update_texcoords_on_vertices(MODEL *model, PointsDrawable *drawable, const std::string& name) {
                auto texcoord = model->template get_vertex_property<vec2_>(name);
                if (texcoord)
            ...
            void update_texcoords_on_edges(MODEL *model, LinesDrawable *drawable, const std::string& name) {
                auto texcoord = model->template get_edge_property<vec2_>(name);
                if (texcoord)
        ...
        void update(PointCloud *model, LinesDrawable *drawable, const std::string &field, float scale) {
            ...
            auto prop = model->get_vertex_property<vec3_>(field);
            if (!prop) {
            ...
            std::vector<vec3_> vertices(model->n_vertices() * 2, vec3_(0.0f, 0.0f, 0.0f));
            int idx = 0;
            for (auto v: model->vertices()) {
        ...
        void update(SurfaceMesh *model, LinesDrawable *drawable, const std::string &field, State::Location location, float scale) {
            ...
            switch (location) {
                case State::FACE: {   // on faces
                    auto prop = model->get_face_property<vec3_>(field);
                    d_points.resize(model->n_faces() * 2, vec3_(0.0f, 0.0f, 0.0f));
                    int idx = 0;
                    for (auto f: model->faces()) {
                        int size = 0;
                        for (auto v: model->vertices(f)) {
                ...
                case State::VERTEX: {   // on vertices
                    auto prop = model->get_vertex_property<vec3_>(field);
                    d_points.resize(model->n_vertices() * 2, vec3_(0.0f, 0.0f, 0.0f));
                    for (auto v: model->vertices()) {
                ...
                case State::EDGE: {   // on edges
                    auto prop = model->get_edge_property<vec3_>(field);
                    d_points.resize(model->n_edges() * 2, vec3_(0.0f, 0.0f, 0.0f));
                    for (auto e : model->edges()) {
        ...
        void update(SurfaceMesh *model, TrianglesDrawable *drawable) {
            ...
            switch (drawable->coloring_method()) {
                case State::TEXTURED: {
                    switch (drawable->property_location()) {
                        case State::VERTEX: {
                            auto texcoord = model->get_vertex_property<vec2_>(name);
                            if (texcoord)
                                internal::update_texcoords_on_vertices(model, drawable, texcoord);
                            else {
                                LOG(WARNING) << "texcoord property \'" << name
                        ...
                        case State::HALFEDGE: {
                            auto texcoord = model->get_halfedge_property<vec2_>(name);
                            if (texcoord)
                ...
                case State::COLOR_PROPERTY: {
                    switch (drawable->property_location()) {
                        case State::FACE: {
                            auto colors = model->get_face_property<vec3_>(name);
                            if (colors)
                        ...
                        case State::VERTEX: {
                            auto colors = model->get_vertex_property<vec3_>(name);
                            if (colors)
                                internal::update_colors_on_vertices(model, drawable, colors);
                            else {
                                LOG(WARNING) << "color property \'" << name
