...
namespace easy3d {
    ...
    namespace shape {
         ...
         * \details The grid is centered at vec3_(0, 0, 0) and lies on the XOY plane.
         ...
         */
        void create_grid(int x_steps, int y_steps, std::vector<vec3_> &points, float depth = 0.0f, float scale = 0.5f);

        /**
         * \brief Generates data for a unit circle as a set of line segments.
		 * \details The circle is centered at vec3_(0, 0, 0) and lies on the XOY plane.
         ...
         * \details The box is centered at vec3_(0, 0, 0) and is aligned with the main axes.
         * \param abstracted \c true to draw an abstracted version (only part of its corners).
         */
        void create_box(std::vector<vec3_> &points, std::vector<vec3_> &colors, bool abstracted = false);
        ...
        void create_cylinder(
                double radius, int slices, const vec3_ &s, const vec3_ &t, const vec3_ &color,
                std::vector<vec3_> &points, std::vector<vec3_> &normals, std::vector<vec3_> &colors
        );
         ...
         */
        void create_camera(std::vector<vec3_> &points, float width, float fov, float hw_ratio = 0.6f);
...
//        void create_image_plane(const easy3d::mat3 &R, const easy3d::vec3_ &t) {
//            if (!image_plane_) {
//                image_plane_ = new TrianglesDrawable("image_plane");
//                image_plane_->set_visible(show_images_);
//                image_plane_->update_texcoord_buffer({vec2_(0, 0), vec2_(1, 0), vec2_(1, 1), vec2_(0, 1)});
...
//            const vec3_ c(0.0f, 0.0f, 0.0f);  // camera center
//            std::vector<vec3_> corners = {
//                    vec3_(-halfWidth, -halfHeight, -dist),
//                    vec3_(halfWidth, -halfHeight, -dist),
//                    vec3_(halfWidth, halfHeight, -dist),
//                    vec3_(-halfWidth, halfHeight, -dist)
//            };
