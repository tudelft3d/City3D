...
namespace easy3d {
    ...
    /*! \brief The Frame class represents a coordinate system, defined by a position
      ...
      Frame fr(vec3_(0.5,0,0), quat(vec3_(0,1,0), vec3_(1,1,1)));
      glPushMatrix();
    ...
    public:
        ...
        //@{
        Frame(const vec3_ &position, const quat &orientation);

        void setPosition(const vec3_ &position);

        void setPositionWithConstraint(vec3_ &position);

        void setOrientation(const quat &orientation);

        void setOrientationWithConstraint(quat &orientation);

        void setPositionAndOrientation(const vec3_ &position,
                                       const quat &orientation);

        void setPositionAndOrientationWithConstraint(vec3_ &position,
                                                     quat &orientation);

        vec3_ position() const;
    ...
    public:
        ...
        of the Frame. */
        void setTranslation(const vec3_ &translation) {
            t_ = translation;
            modified.send();
        }

        void setTranslationWithConstraint(vec3_ &translation);
        ...
        void setTranslationAndRotation(const vec3_ &translation, const quat &rotation);

        void setTranslationAndRotationWithConstraint(vec3_ &translation, quat &rotation);
        ...
        vec3_ translation() const { return t_; }
    ...
    public:
        ...
        //@{
        void translate(vec3_ &t);

        void translate(const vec3_ &t);

        void rotate(quat &q);

        void rotate(const quat &q);

        void rotateAroundPoint(quat &rotation, const vec3_ &point);

        void rotateAroundPoint(const quat &rotation, const vec3_ &point);

        void alignWithFrame(const Frame *const frame, bool move = false,
                            float threshold = 0.0f);

        void projectOnLine(const vec3_ &origin, const vec3_ &direction);
        //@}

        /*! @name Coordinate system transformation of 3D coordinates */
        //@{
        vec3_ coordinatesOf(const vec3_ &src) const;

        vec3_ inverseCoordinatesOf(const vec3_ &src) const;

        vec3_ localCoordinatesOf(const vec3_ &src) const;

        vec3_ localInverseCoordinatesOf(const vec3_ &src) const;

        vec3_ coordinatesOfIn(const vec3_ &src, const Frame *const in) const;

        vec3_ coordinatesOfFrom(const vec3_ &src, const Frame *const from) const;
        //@}
        ...
        // the identity.
        //@{
        vec3_ transformOf(const vec3_ &src) const;

        vec3_ inverseTransformOf(const vec3_ &src) const;

        vec3_ localTransformOf(const vec3_ &src) const;

        vec3_ localInverseTransformOf(const vec3_ &src) const;

        vec3_ transformOfIn(const vec3_ &src, const Frame *const in) const;

        vec3_ transformOfFrom(const vec3_ &src, const Frame *const from) const;
        //@}
