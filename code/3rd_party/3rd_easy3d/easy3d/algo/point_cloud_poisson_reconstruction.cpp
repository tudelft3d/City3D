...
#define BType  BOUNDARY_NEUMANN
    ...
    namespace internal {
        ...
        convert_to_mesh(CoredFileMeshData<Vertex> &mesh, const XForm4x4<REAL> &iXForm,
            ...
            SurfaceMesh::VertexProperty<vec3_> color;
            if (has_colors)
                color = result->add_vertex_property<vec3_>("v:color");
            ...
            for (std::size_t i = 0; i < num_ic_pts; ++i) {
                ...
                SurfaceMesh::Vertex vv = result->add_vertex(vec3_(pt.coords[0], pt.coords[1], pt.coords[2]));
    ...
    SurfaceMesh *PoissonReconstruction::apply(const PointCloud *cloud, const std::string &density_attr_name) const {
        ...
        PointCloud::VertexProperty<vec3_> normals = cloud->get_vertex_property<vec3_>("v:normal");
        if (!normals) {
        ...
        { // Load the samples (and color data)
            ...
            PointCloud::VertexProperty<vec3_> colors = cloud->get_vertex_property<vec3_>("v:color");
            if (colors) {
                cls = colors.vector()[0];
...
#pragma omp parallel for num_threads(threads_) reduction( + : valueSum, weightSum )
    ...
    SurfaceMesh *PoissonReconstruction::trim(
        ...
        auto color = mesh->get_vertex_property<vec3_>("v:color");
...
//	StopWatch t; t.start();
        ...
        if (has_color)
            color = trimmed_mesh->add_vertex_property<vec3_>("v:color");

        std::vector<SurfaceMesh::Vertex> all_vertices;
        for (auto vert : vertices) {
            const Point3D<REAL> &pt = vert.point;
            SurfaceMesh::Vertex v = trimmed_mesh->add_vertex(vec3_(pt.coords[0], pt.coords[1], pt.coords[2]));
            all_vertices.push_back(v);
            density[v] = vert.value;

            if (has_color) {
                unsigned char *c = vert.color;
                color[v] = vec3_(static_cast<float>(c[0]) / 255.0f, static_cast<float>(c[1]) / 255.0f, static_cast<float>(c[2]) / 255.0f);
            }
        }
