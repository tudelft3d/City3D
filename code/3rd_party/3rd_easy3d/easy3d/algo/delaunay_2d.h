...
namespace easy3d {
    ...
    public:
        ...
        void set_vertices(const std::vector<vec2_> &vertices) {
        ...
        unsigned int nearest_vertex(const vec2_ &p) const {
            return nearest_vertex(p.data());
        }

        const vec2_ &vertex(unsigned int i) const {
            return *(const vec2_ *) vertex_ptr(i);
        }
