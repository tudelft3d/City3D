...
//                const Plane3 plane(vec3_(p.getValue()), vec3_(n.getValue()));
                        for (auto id : vts) {
...
//                const vec3_  position(pos[0], pos[1], pos[2]);
//                vec3_  dir(nor[0], nor[1], nor[2]); dir = normalize(dir);
                        for (auto id : vts) {
                            const PointCloud::Vertex v(id);
...
//                double max_radius = torus.MajorRadius();
    ...
    int PrimitivesRansac::detect(
        ...
        const std::vector<vec3_> &nms = normals.vector();
...
#pragma omp parallel for
        for (int index = 0; index < vertices.size(); ++index) {
            ...
            const vec3_ &p = pts[idx];
            const vec3_ &n = nms[idx];
            pc[index] = Point(
                    Vec3f(p.x, p.y, p.z),
                    Vec3f(n.x, n.y, n.z)
            );
