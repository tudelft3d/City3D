...
namespace easy3d {
    ...
    SurfaceMeshHoleFilling::SurfaceMeshHoleFilling(SurfaceMesh *mesh) : mesh_(mesh) {
        points_ = mesh_->get_vertex_property<vec3_>("v:point");
    }
    ...
    vec3_ SurfaceMeshHoleFilling::compute_normal(SurfaceMesh::Vertex _a, SurfaceMesh::Vertex _b,
    ...
    float SurfaceMeshHoleFilling::compute_angle(const vec3_ &_n1,
                                                const vec3_ &_n2) const {
    ...
    SurfaceMeshHoleFilling::Weight SurfaceMeshHoleFilling::compute_weight(int _i, int _j,
        ...
        float angle(0);
        const vec3_ n = compute_normal(a, b, c);
        ...
        for (ok = false, i = 0; !ok && i < 10; ++i) {
            ...
            for (auto e : mesh_->edges()) {
                if (!elocked_[e]) {
                    ...
                    const vec3_ &p0 = points_[mesh_->target(h10)];
                    const vec3_ &p1 = points_[mesh_->target(h01)];
    ...
    void SurfaceMeshHoleFilling::relaxation() {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            points_[vertices[i]] = vec3_(static_cast<float>(tmp(0)), static_cast<float>(tmp(1)), static_cast<float>(tmp(2)));
        }
