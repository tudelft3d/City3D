...
namespace easy3d {
    ...
    bool SurfaceMeshParameterization::setup_boundary_constraints() {
        // get properties
        auto points = mesh_->vertex_property<vec3_>("v:point");
        auto tex = mesh_->vertex_property<vec2_>("v:texcoord");
    ...
    void SurfaceMeshParameterization::harmonic(bool use_uniform_weights) {
        ...
        if (solver.info() != Eigen::Success) {
            ...
            for (i = 0; i < n; ++i) {
                ...
                tex[free_vertices[i]] = vec2_(tmp(0), tmp(1));
            }
        }
    ...
    bool SurfaceMeshParameterization::setup_lscm_boundary() {
        ...
        // vertex properties
        auto pos = mesh_->vertex_property<vec3_>("v:point");
        auto tex = mesh_->vertex_property<vec2_>("v:texcoord");
        ...
        tex[v1] = vec2_(0.0, 0.0);
        tex[v2] = vec2_(1.0, 1.0);
        locked[v1] = true;
        locked[v2] = true;
    ...
    void SurfaceMeshParameterization::lscm() {
        ...
        if (solver.info() != Eigen::Success) {
            ...
            for (unsigned int i = 0; i < n; ++i) {
                tex[free_vertices[i]] = vec2_(x[i], x[i + n]);
            }
        }

        // scale tex coordiantes to unit square
        vec2_ bbmin(1, 1), bbmax(0, 0);
        for (auto v : mesh_->vertices()) {
