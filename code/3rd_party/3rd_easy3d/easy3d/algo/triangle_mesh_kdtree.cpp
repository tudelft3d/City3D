...
namespace easy3d {
    ...
    TriangleMeshKdTree::TriangleMeshKdTree(const SurfaceMesh *mesh, unsigned int max_faces,
        ...
        SurfaceMesh::VertexProperty<vec3_> points = mesh->get_vertex_property<vec3_>("v:point");
    ...
    unsigned int TriangleMeshKdTree::build_recurse(Node *node, unsigned int max_faces,
        ...
        vec3_ bb = bbox.max_point() - bbox.min_point();
        float length = bb[0];
        int axis = 0;
        if (bb[1] > length)
            length = bb[(axis = 1)];
    ...
    TriangleMeshKdTree::NearestNeighbor TriangleMeshKdTree::nearest(const vec3_ &p) const {
    ...
    void TriangleMeshKdTree::nearest_recurse(Node *node, const vec3_ &point,
