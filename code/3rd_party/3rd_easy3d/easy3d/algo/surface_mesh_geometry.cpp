...
namespace easy3d {
    ...
    namespace geom {
        ...
        vec3_ centroid(const SurfaceMesh *mesh, SurfaceMesh::Face f) {
            vec3_ c(0, 0, 0);
            float n(0);
        ...
        vec3_ centroid(const SurfaceMesh *mesh) {
            vec3_ center(0, 0, 0), c;
            float area(0), a;
            for (auto f : mesh->faces()) {
        ...
        vec3_ laplace(const SurfaceMesh *mesh, SurfaceMesh::Vertex v) {
            vec3_ laplace(0.0f, 0.0f, 0.0f);
        ...
        double angle_sum(const SurfaceMesh *mesh, SurfaceMesh::Vertex v) {
            ...
            if (!mesh->is_border(v)) {
                const vec3_ &p0 = mesh->position(v);

                for (auto h : mesh->halfedges(v)) {
                    const vec3_ &p1 = mesh->position(mesh->target(h));
                    const vec3_ &p2 =
                            mesh->position(mesh->target(mesh->prev_around_source(h)));

                    const vec3_ p01 = normalize(p1 - p0);
                    const vec3_ p02 = normalize(p2 - p0);
