...
namespace easy3d {
    ...
    SurfaceMesh SurfaceMeshFactory::tetrahedron() {
        ...
        auto v0 = mesh.add_vertex(vec3_(0, 0, 1));
        auto v1 = mesh.add_vertex(vec3_(-c, d, -a));
        auto v2 = mesh.add_vertex(vec3_(-c, -d, -a));
        auto v3 = mesh.add_vertex(vec3_(b, 0, -a));
    ...
    SurfaceMesh SurfaceMeshFactory::hexahedron() {
        ...
        auto v0 = mesh.add_vertex(vec3_(-a, -a, -a));
        auto v1 = mesh.add_vertex(vec3_(a, -a, -a));
        auto v2 = mesh.add_vertex(vec3_(a, a, -a));
        auto v3 = mesh.add_vertex(vec3_(-a, a, -a));
        auto v4 = mesh.add_vertex(vec3_(-a, -a, a));
        auto v5 = mesh.add_vertex(vec3_(a, -a, a));
        auto v6 = mesh.add_vertex(vec3_(a, a, a));
        auto v7 = mesh.add_vertex(vec3_(-a, a, a));
    ...
    SurfaceMesh SurfaceMeshFactory::icosahedron() {
        ...
        auto v1 = mesh.add_vertex(vec3_(0, b, -a));
        auto v2 = mesh.add_vertex(vec3_(b, a, 0));
        auto v3 = mesh.add_vertex(vec3_(-b, a, 0));
        auto v4 = mesh.add_vertex(vec3_(0, b, a));
        auto v5 = mesh.add_vertex(vec3_(0, -b, a));
        auto v6 = mesh.add_vertex(vec3_(-a, 0, b));
        auto v7 = mesh.add_vertex(vec3_(0, -b, -a));
        auto v8 = mesh.add_vertex(vec3_(a, 0, -b));
        auto v9 = mesh.add_vertex(vec3_(a, 0, b));
        auto v10 = mesh.add_vertex(vec3_(-a, 0, -b));
        auto v11 = mesh.add_vertex(vec3_(b, -a, 0));
        auto v12 = mesh.add_vertex(vec3_(-b, -a, 0));
    ...
    SurfaceMesh SurfaceMeshFactory::uv_sphere(const vec3_ &center, float radius,
                                              size_t n_slices, size_t n_stacks) {
        SurfaceMesh mesh;

        // add top vertex
        auto v0 = mesh.add_vertex(vec3_(center[0], center[1] + radius, center[2]));
        ...
        for (size_t i = 0; i < n_stacks - 1; i++) {
            ...
            for (size_t j = 0; j < n_slices; ++j) {
                ...
                mesh.add_vertex(vec3_(static_cast<float>(x), static_cast<float>(y), static_cast<float>(z)));
            }
        }

        // add bottom vertex
        auto v1 = mesh.add_vertex(vec3_(center[0], center[1] - radius, center[2]));
    ...
    SurfaceMesh SurfaceMeshFactory::cone(size_t n_subdivisions, float radius, float height) {
        ...
        for (size_t i = 0; i < n_subdivisions; i++) {
            ...
            auto v = mesh.add_vertex(vec3_(static_cast<float>(x), static_cast<float>(y), 0.0f));
            base_vertices.push_back(v);
        }

        // add the tip of the cone
        auto v0 = mesh.add_vertex(vec3_(0.0, 0.0, height));
    ...
    SurfaceMesh SurfaceMeshFactory::cylinder(size_t n_subdivisions, float radius, float height) {
        ...
        for (size_t i = 0; i < n_subdivisions; i++) {
            ...
            v = mesh.add_vertex(vec3_(static_cast<float>(x), static_cast<float>(y), height));
