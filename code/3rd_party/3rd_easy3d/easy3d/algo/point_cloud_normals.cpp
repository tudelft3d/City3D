...
namespace easy3d {
    ...
    bool PointCloudNormals::estimate(PointCloud *cloud, unsigned int k /* = 16 */,
        ...
        std::vector<vec3_> &normals = cloud->vertex_property<vec3_>("v:normal").vector();
...
#pragma omp parallel for
    ...
    namespace internal {
        ...
        void build_graph(easy3d::PointCloud *cloud, const KdTreeSearch *tree, unsigned int k, RiemannianGraph &graph) {
            ...
            auto points = cloud->get_vertex_property<easy3d::vec3_>("v:point");
            auto normals = cloud->get_vertex_property<easy3d::vec3_>("v:normal");
            for (auto v : cloud->vertices()) {
                const easy3d::vec3_ &p = points[v];
                ...
                for (auto index : neighbor_indices) {
                    ...
                    const easy3d::vec3_ &n1 = normals[v];
                    const easy3d::vec3_ &n2 = normals[v2];
        ...
        public:
            explicit MST_Graph(PointCloud::VertexProperty<vec3_> nmap) : normal_map(nmap) {}

            PointCloud::VertexProperty<vec3_> normal_map;
        ...
        void find_top_vertex(PointCloud *cloud, RiemannianGraph &graph) {
            ...
            if (top.idx() != -1) {
                auto normals = cloud->get_vertex_property<vec3_>("v:normal");
                if (normals[top].z < 0)
        ...
        void propagate_normal(MST_Graph::Edge edge, MST_Graph &graph) {
            ...
            const vec3_ &source_normal = graph.normal_map[v_source];
            ...
            vec3_ &target_normal = graph.normal_map[v_target];
    ...
    bool PointCloudNormals::reorient(PointCloud *cloud, unsigned int k) {
        ...
        for (const auto &graph : components) {
            ms_trees.emplace_back(cloud->get_vertex_property<vec3_>("v:normal"));
        ...
        if (cloud->renderer()) {
            ...
            auto point_prop = cloud->get_vertex_property<vec3_>("v:point");
            std::vector<vec3_> points, colors;
            std::cout << "num MST: " << ms_trees.size() << std::endl;
            for (const auto &mst: ms_trees) {
                const vec3_ &c = random_color(); // give each mst a unique color
