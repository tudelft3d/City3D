...
namespace easy3d {
    ...
    float SurfaceMeshComponent::area() const {
        auto normals = mesh_->get_face_property<vec3_>("f:normal");
        if (!normals) {
            mesh_->update_face_normals();
            normals = mesh_->get_face_property<vec3_>("f:normal");
        }

        auto points = mesh_->get_vertex_property<vec3_>("v:point");
        ...
        for (const auto& t : triangles) {
            const vec3_ a(vts[t[0]]->data());
            const vec3_ b(vts[t[1]]->data());
            const vec3_ c(vts[t[2]]->data());
    ...
    void SurfaceMeshComponent::translate(const vec3_ &offset) {
        auto points = mesh_->get_vertex_property<vec3_>("v:point");
        for (auto v : vertices_)
