...
namespace easy3d {
    ...
    SurfaceMeshSimplification::SurfaceMeshSimplification(SurfaceMesh *mesh)
        ...
        // get properties
        vpoint_ = mesh_->get_vertex_property<vec3_>("v:point");

        // compute face normals
        mesh_->update_face_normals();
        fnormal_ = mesh_->get_face_property<vec3_>("f:normal");
    }
    ...
    bool SurfaceMeshSimplification::is_collapse_legal(const CollapseData &cd) {
        ...
        const vec3_ p0 = vpoint_[cd.v0];
        const vec3_ p1 = vpoint_[cd.v1];
        ...
        if (normal_deviation_ == 0.0) {
            ...
            for (auto f : mesh_->faces(cd.v0)) {
                if (f != cd.fl && f != cd.fr) {
                    vec3_ n0 = fnormal_[f];
                    vec3_ n1 = mesh_->compute_face_normal(f);
                    if (dot(n0, n1) < 0.0) {
                        vpoint_[cd.v0] = p0;
                        return false;
                    }
    ...
    float SurfaceMeshSimplification::aspect_ratio(SurfaceMesh::Face f) const {
        ...
        const vec3_ p0 = vpoint_[*fvit];
        const vec3_ p1 = vpoint_[*(++fvit)];
        const vec3_ p2 = vpoint_[*(++fvit)];

        const vec3_ d0 = p0 - p1;
        const vec3_ d1 = p1 - p2;
        const vec3_ d2 = p2 - p0;
    ...
    float SurfaceMeshSimplification::distance(SurfaceMesh::Face f, const vec3_ &p) const {
        SurfaceMesh::VertexAroundFaceCirculator fvit = mesh_->vertices(f);
