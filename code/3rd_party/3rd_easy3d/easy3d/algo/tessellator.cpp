...
namespace easy3d {
...
#ifndef NDEBUG
                    if (distance2(vec3_(unique_vertices_[pos->second]->data()), vec3_(v.data())) > 1e-6) {
                        LOG_N_TIMES(3, ERROR) << "bad: two distinct points have the same hash key\n"
                                              << "\t\tpoint: " << vec3_(unique_vertices_[pos->second]->data()) << " <-> "
    ...
    void Tessellator::begin_polygon(const vec3_ &normal) {
        num_elements_in_polygon_ = 0;
    ...
    void Tessellator::add_vertex(const vec3_ &xyz, int idx) {
        add_vertex(Vertex(xyz, idx));
    }


    void Tessellator::add_vertex(const vec3_ &xyz, const vec2_ &t, int idx) {
        Vertex v(xyz, idx);
        v.append(t);
        add_vertex(v);
    }
    ...
    void Tessellator::add_vertex(const vec3_ &xyz, const vec3_ &v1, int idx) {
        Vertex v(xyz, idx);
        v.append(v1);
        add_vertex(v);
    }
    ...
    void Tessellator::add_vertex(const vec3_ &xyz, const vec3_ &v1, const vec2_ &t, int idx) {
        Vertex v(xyz, idx);
        v.append(v1);
        v.append(t);
        add_vertex(v);
    ...
    void Tessellator::add_vertex(const vec3_ &xyz, const vec3_ &v1, const vec3_ &v2, int idx) {
        Vertex v(xyz, idx);
        v.append(v1);
        v.append(v2);
        add_vertex(v);
    ...
    void Tessellator::add_vertex(const vec3_ &xyz, const vec3_ &v1, const vec3_ &v2, const vec2_ &t, int idx) {
        Vertex v(xyz, idx);
        v.append(v1);
        v.append(v2);
        v.append(t);
