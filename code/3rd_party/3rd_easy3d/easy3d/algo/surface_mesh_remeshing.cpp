...
namespace easy3d {
    ...
    SurfaceMeshRemeshing::SurfaceMeshRemeshing(SurfaceMesh *mesh)
        ...
        points_ = mesh_->vertex_property<vec3_>("v:point");

        mesh_->update_vertex_normals();
        vnormal_ = mesh_->vertex_property<vec3_>("v:normal");
    }
    ...
    void SurfaceMeshRemeshing::preprocessing() {
        ...
        if (use_projection_) {
            ...
            refpoints_ = refmesh_->vertex_property<vec3_>("v:point");
            refnormals_ = refmesh_->vertex_property<vec3_>("v:normal");
    ...
    void SurfaceMeshRemeshing::project_to_reference(SurfaceMesh::Vertex v) {
        ...
        const vec3_ p0 = refpoints_[*fvIt];
        const vec3_ n0 = refnormals_[*fvIt];
        const float s0 = refsizing_[*fvIt];
        ++fvIt;
        const vec3_ p1 = refpoints_[*fvIt];
        const vec3_ n1 = refnormals_[*fvIt];
        const float s1 = refsizing_[*fvIt];
        ++fvIt;
        const vec3_ p2 = refpoints_[*fvIt];
        const vec3_ n2 = refnormals_[*fvIt];
        const float s2 = refsizing_[*fvIt];

        // get barycentric coordinates
        const vec3_ b = geom::barycentric_coordinates(p, p0, p1, p2);
        ...
        for (ok = false, i = 0; !ok && i < 10; ++i) {
            ...
            for (auto e : mesh_->edges()) {
                ...
                if (!elocked_[e] && is_too_long(v0, v1)) {
                    const vec3_ &p0 = points_[v0];
                    const vec3_ &p1 = points_[v1];
    ...
    void SurfaceMeshRemeshing::tangential_smoothing(unsigned int iterations) {
        ...
        // add property
        SurfaceMesh::VertexProperty <vec3_> update = mesh_->add_vertex_property<vec3_>("v:update");
    ...
    vec3_ SurfaceMeshRemeshing::minimize_squared_areas(SurfaceMesh::Vertex v) {
        dmat3 A(0.0), D;
        dvec3 b(0), d, p, q, x;
        double w;
    ...
    vec3_ SurfaceMeshRemeshing::weighted_centroid(SurfaceMesh::Vertex v) {
        auto p = vec3_(0);
        double ww = 0;
