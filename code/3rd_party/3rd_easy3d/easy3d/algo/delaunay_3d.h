...
namespace easy3d {
    ...
    public:
        ...
        void set_vertices(const std::vector<vec3_> &vertices) {
            // for QDel
            if (vertices.capacity() - vertices.size() < 8) {
                const_cast<std::vector<vec3_> &>(vertices).reserve(vertices.size() + 8);
            }
        ...
        unsigned int nearest_vertex(const vec3_ &p) const {
            return nearest_vertex(p.data());
        }

        const vec3_ &vertex(unsigned int i) const {
            return *(const vec3_ *) vertex_ptr(i);
        }
        ...
        vec3_ facet_normal(unsigned int t, unsigned int f) const {
            assert(t < nb_tets());
            assert(f < 4);
            const vec3_ &p1 = vertex(tet_vertex(t, facet_vertex_[f][0]));
            const vec3_ &p2 = vertex(tet_vertex(t, facet_vertex_[f][1]));
            const vec3_ &p3 = vertex(tet_vertex(t, facet_vertex_[f][2]));
            vec3_ result = cross(p2 - p1, p3 - p1);
            return result;
        }

        vec3_ tet_circumcenter(unsigned int t) const {
            const vec3_ &p0 = vertex(tet_vertex(t, 0));
            const vec3_ &p1 = vertex(tet_vertex(t, 1));
            const vec3_ &p2 = vertex(tet_vertex(t, 2));
            const vec3_ &p3 = vertex(tet_vertex(t, 3));
    ...
    public:
        ...
        */
        ...
        void add_to_facet(
                int e_bisector, const vec3_ &v, bool infinite
        ) {
