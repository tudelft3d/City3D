    ...
    TextMesher::_generate_contours(int codepoint, float &x, float &y, int font_size, std::vector<Polygon2> &contours) {
        ...
        while (contour_begin_index < num_verts) {
            ...
            for (long vi = contour_begin_index; vi < current_contour_last_index; ++vi) {
                ...
                const vec2_ p1 = vec2_(static_cast<float>(v1->x) * scale + x, static_cast<float>(v1->y) * scale + y);
                const vec2_ p2 = vec2_(static_cast<float>(v2->x) * scale + x, static_cast<float>(v2->y) * scale + y);
                const vec2_ pc = vec2_(static_cast<float>(v2->cx) * scale + x, static_cast<float>(v2->cy) * scale + y);
                const vec2_ pc1 = vec2_(static_cast<float>(v2->cx1) * scale + x, static_cast<float>(v2->cy1) * scale + y);
