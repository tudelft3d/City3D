...
namespace easy3d {
    ...
    PointCloud *SurfaceMeshSampler::apply(const SurfaceMesh *input_mesh, int expected_num /* = 1000000 */) {
        auto func = [](const SurfaceMesh *mesh, int num) -> PointCloud * {
            ...
            auto mesh_points = mesh->get_vertex_property<vec3_>("v:point");
            ...
            auto mesh_vertex_normals = mesh->get_vertex_property<vec3_>("v:normal");
            ...
            std::vector<vec3_> triangle_normals;

            auto mesh_face_normals = mesh->get_face_property<vec3_>("f:normal");
            float surface_area = 0.0f;
            for (auto f : mesh->faces()) {
                const vec3_ &n = mesh_face_normals[f];
            ...
            for (std::size_t idx = 0; idx < triangle_num; ++idx) {
                ...
                const vec3_ &n = triangle_normals[idx];
