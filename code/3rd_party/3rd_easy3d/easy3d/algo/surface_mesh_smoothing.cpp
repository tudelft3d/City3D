...
namespace easy3d {
    ...
    void SurfaceMeshSmoothing::explicit_smoothing(unsigned int iters,
        ...
        auto laplace = mesh_->add_vertex_property<vec3_>("v:laplace");
    ...
    void SurfaceMeshSmoothing::implicit_smoothing(float timestep,
        ...
        if (solver.info() != Eigen::Success) {
            ...
            for (i = 0; i < n; ++i) {
                ...
                points[free_vertices[i]] = vec3_(static_cast<float>(tmp(0)), static_cast<float>(tmp(1)), static_cast<float>(tmp(2)));
            }
        }
        ...
        if (rescale) {
            ...
            vec3_ center_after = geom::centroid(mesh_);
            vec3_ trans = center_before - center_after;
            for (auto v : mesh_->vertices())
