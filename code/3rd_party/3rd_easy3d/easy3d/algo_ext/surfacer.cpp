...
namespace easy3d {
    ...
    namespace internal {
        ...
        void to_cgal(SurfaceMesh *input, CGALMesh &output) {
            ...
            for (auto v : input->vertices()) {
                const vec3_ &p = input->position(v);
        ...
        void to_cgal(const std::vector<vec3_> &input_points,
        ...
        void to_easy3d(const std::vector<K::Point_3> &input_points,
            ...
            for (std::size_t i = 0; i < input_points.size(); ++i) {
                ...
                points[i] = vec3_({static_cast<float>(CGAL::to_double(p.x())),
                                  static_cast<float>(CGAL::to_double(p.y())),
                                  static_cast<float>(CGAL::to_double(p.z()))});
            }
        ...
        void remove_duplicate_vertices(std::vector<vec3_> &points,
                                       std::vector<Surfacer::Polygon> &polygons) {
            ...
            public:
                ...
                bool operator()(const vec3_ &v0, const vec3_ &v1) const {
            ...
            std::map<vec3_, int, CompareVec> vMap(comp);
            std::map<vec3_, int, CompareVec>::iterator vMapIt;
            std::map<int, int> index_map;

            std::vector<vec3_> result_points = points;
            ...
            for (std::size_t i = 0; i < result_points.size(); ++i) {
                ...
                if (vMapIt == vMap.end()) {
                    ...
                    points.emplace_back(vec3_(p.x, p.y, p.z));
                } else {
        ...
        void to_polygon_soup(SurfaceMesh *mesh, std::vector<vec3_> &points,
                             std::vector<Surfacer::Polygon> &polygons) {
            if (!mesh)
                return;
        ...
        void to_polygon_mesh(const std::vector<vec3_> &points,
    ...
    bool Surfacer::orient_and_stitch_polygon_soup(std::vector<vec3_> &input_points, std::vector<Polygon> &input_polygons) {
    ...
    void Surfacer::repair_polygon_soup(std::vector<vec3_> &input_points, std::vector<Polygon> &input_polygons) {
        const auto num_vertices = input_points.size();
        const auto num_faces = input_polygons.size();
    ...
    unsigned int Surfacer::remove_degenerate_faces(SurfaceMesh *mesh, float length_threshold) {
        ...
        auto prop = mesh->get_vertex_property<vec3_>("v:point");
