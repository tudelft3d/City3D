...
namespace easy3d {
        ...
        , model_idx_(-1)
    {
        ...
        camera_->setUpVector(vec3_(0, 0, 1)); // Z pointing up
        camera_->setViewDirection(vec3_(-1, 0, 0)); // X pointing out
    ...
    bool Viewer::mouse_press_event(int x, int y, int button, int modifiers) {
        ...
        if (pressed_key_ == GLFW_KEY_Z || modifiers == GLFW_MOD_SHIFT) {
            if (button == GLFW_MOUSE_BUTTON_LEFT) {
                bool found = false;
                const vec3_ &p = point_under_pixel(x, y, found);
                if (found) {
                    ...
                    if (pressed_key_ == GLFW_KEY_Z &&  modifiers == 0) {
                        ...
                        const vec3_& pos = coef * camera()->frame()->position() + (1.0f - coef)*p;
    ...
    bool Viewer::key_press_event(int key, int modifiers) {
        ...
        if (key == GLFW_KEY_F1 && modifiers == 0)
            ...
            camera_->frame()->translate(camera_->frame()->inverseTransformOf(vec3_(0.0, 0.0, -step)));
            ...
            float step = 0.05f * camera_->sceneRadius();
            camera_->frame()->translate(camera_->frame()->inverseTransformOf(vec3_(-step, 0.0, 0.0)));
            ...
            float step = 0.05f * camera_->sceneRadius();
            camera_->frame()->translate(camera_->frame()->inverseTransformOf(vec3_(0.0, step, 0.0)));
    ...
    vec3_ Viewer::point_under_pixel(int x, int y, bool &found) const {
        ...
        found = depth < 1.0f;
        if (found) {
            vec3_ point(static_cast<float>(x), static_cast<float>(y), depth);
    ...
    void Viewer::draw_corner_axes() const {
        ...
        if (!drawable_axes_) {
            ...
            std::vector<vec3_> points, normals, colors;
            shape::create_cylinder(0.03, 10, vec3_(0, 0, 0), vec3_(base, 0, 0), vec3_(1, 0, 0), points, normals, colors);
            shape::create_cylinder(0.03, 10, vec3_(0, 0, 0), vec3_(0, base, 0), vec3_(0, 1, 0), points, normals, colors);
            shape::create_cylinder(0.03, 10, vec3_(0, 0, 0), vec3_(0, 0, base), vec3_(0, 0, 1), points, normals, colors);
            shape::create_cone(0.06, 20, vec3_(base, 0, 0), vec3_(base + head, 0, 0), vec3_(1, 0, 0), points, normals,
                               colors);
            shape::create_cone(0.06, 20, vec3_(0, base, 0), vec3_(0, base + head, 0), vec3_(0, 1, 0), points, normals,
                               colors);
            shape::create_cone(0.06, 20, vec3_(0, 0, base), vec3_(0, 0, base + head), vec3_(0, 0, 1), points, normals,
                               colors);
            shape::create_sphere(vec3_(0, 0, 0), 0.06, 20, 20, vec3_(0, 1, 1), points, normals, colors);
        ...
        const vec3_ &wCamPos = camera_->position();
        // it can also be computed as follows:
        //const vec3_& wCamPos = invMV * vec4(0, 0, 0, 1);
    ...
    void Viewer::post_draw() {
        ...
        if (show_pivot_point_) {
            ...
            const vec3_ &pivot = camera()->projectedCoordinatesOf(camera()->pivotPoint());
            const std::vector<vec3_> points = {
                    vec3_(pivot.x - size, pivot.y, 0.5f), vec3_(pivot.x + size, pivot.y, 0.5f),
                    vec3_(pivot.x, pivot.y - size, 0.5f), vec3_(pivot.x, pivot.y + size, 0.5f)
            };
    ...
    void Viewer::draw_face_labels(Model *model, TextRenderer *texter, int font_id, const vec3_ &color) const {
        ...
        for (auto f : mesh->faces()) {
            ...
            const vec3_ p = camera()->projectedCoordinatesOf(c);
    ...
    void Viewer::draw_vertex_labels(Model *model, TextRenderer *texter, int font_id, const vec3_ &color) const {
        auto mesh = dynamic_cast<SurfaceMesh *>(model);
        const auto &points = mesh->points();
        for (std::size_t id = 0; id < points.size(); ++id) {
            const vec3_& v = points[id];
            const vec3_ p = camera()->projectedCoordinatesOf(v);
            const float x = p.x * dpi_scaling_;
            const float y = (static_cast<float>(height()) - 1.0f - p.y) * dpi_scaling_;
    ...
    void Viewer::copy_view() {
        const vec3_ pos = camera()->position();
...
//        glScissor(scissor[0], scissor[1], scissor[2], scissor[3]);
        ...
        if (dynamic_cast<SurfaceMesh *>(current_model()) && texter_ && texter_->num_fonts() > 1) {
            draw_face_labels(current_model(), texter_, 1, vec3_(0, 0, 1));
            draw_vertex_labels(current_model(), texter_, 1, vec3_(0, 1, 0));
        }
#endif
    }
