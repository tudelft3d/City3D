	...
	namespace io {
		...
		bool load_xyz(const std::string& file_name, PointCloud* cloud) {
            ...
            if (Translator::instance()->status() == Translator::DISABLED) {
                ...
                for (const auto &p: points)
                    cloud->add_vertex(vec3_(static_cast<float>(p.x - origin.x), static_cast<float>(p.y - origin.y), static_cast<float>(p.z - origin.z)));
		...
		bool save_bxyz(const std::string& file_name, const PointCloud* cloud) {
            ...
            if (trans) { // has translation
                ...
                output.write((char*)points.data(), static_cast<long>(cloud->n_vertices() * sizeof(vec3_)));
			return true;
		}
