...
namespace easy3d {
    ...
    namespace io {
        ...
        bool PointCloudIO_vg::save_vg(const std::string& file_name, const PointCloud* cloud) {
            ...
            const std::vector<vec3_>& points = cloud->points();
            auto cls = cloud->get_vertex_property<vec3_>("v:color");
            auto nms = cloud->get_vertex_property<vec3_>("v:normal");
            ...
            if (cls) {
                const std::vector<vec3_>& colors = cls.vector();
            ...
            if (nms) {
                const std::vector<vec3_>& normals = nms.vector();
        ...
        */
        ...
        bool PointCloudIO_vg::load_vg(const std::string& file_name, PointCloud* cloud) {
            ...
            std::vector<vec3_>& points = cloud->points();
            ...
            if (Translator::instance()->status() == Translator::DISABLED) {
                ...
                for (std::size_t i = 1; i < num; ++i) {
                    input >> x >> y >> z;
                    points[i] = vec3_(static_cast<float>(x - x0), static_cast<float>(y - y0), static_cast<float>(z - z0));
                    if (input.fail()) {
                        LOG(ERROR) << "failed reading the " << i << "_th point";
                        return false;
                    }
                ...
                for (std::size_t i = 0; i < num; ++i) {
                    input >> x >> y >> z;
                    points[i] = vec3_(static_cast<float>(x - origin.x), static_cast<float>(y - origin.y), static_cast<float>(z - origin.z));
                    if (input.fail()) {
                        LOG(ERROR) << "failed reading the " << i << "_th point";
                        return false;
                    }
            ...
            if (num == points.size()) {
                auto cls = cloud->add_vertex_property<vec3_>("v:color");
                std::vector<vec3_>& colors = cls.vector();
                for (std::size_t i = 0; i < num; ++i) {
                    input >> colors[i];
                    if (input.fail()) {
            ...
            if (num == points.size()) {
                auto nms = cloud->add_vertex_property<vec3_>("v:normal");
                std::vector<vec3_>& normals = nms.vector();
                for (std::size_t i = 0; i < num; ++i) {
                    input >> normals[i];
                    if (input.fail()) {
        ...
        bool PointCloudIO_vg::load_bvg(const std::string& file_name, PointCloud* cloud) {
            ...
            auto points = cloud->get_vertex_property<vec3_>("v:point");
            input.read((char*)points.data(), static_cast<long>(num * sizeof(vec3_)));

            if (Translator::instance()->status() == Translator::TRANSLATE_USE_FIRST_POINT) {
                auto& positions = points.vector();

                // the first point
                const vec3_ p0 = positions[0];
                ...
                const dvec3 &origin = Translator::instance()->translation();
                auto& pts = cloud->get_vertex_property<vec3_>("v:point").vector();
                for (auto& p: pts) {
            ...
            if (num == cloud->n_vertices()) {
                auto colors = cloud->add_vertex_property<vec3_>("v:color");
                input.read((char*)colors.data(), static_cast<long>(num * sizeof(vec3_)));
            }

            // read the normals block if exists
            input.read((char*)(&num), sizeof(int));
            if (num == cloud->n_vertices()) {
                auto normals = cloud->add_vertex_property<vec3_>("v:normal");
                input.read((char*)normals.data(), static_cast<long>(num * sizeof(vec3_)));
            }
        ...
        bool PointCloudIO_vg::save_bvg(const std::string& file_name, const PointCloud* cloud) {
            ...
            if (trans) { // has translation
                ...
                output.write((char*)points.data(), static_cast<long>(num * sizeof(vec3_)));

            if (cls) {
                const std::vector<vec3_>& colors = cls.vector();
                output.write((char*)&num, sizeof(int));
                output.write((char*)colors.data(), static_cast<long>(num * sizeof(vec3_)));
            }
            else {
                int dummy = 0;
            ...
            if (nms) {
                const std::vector<vec3_>& normals = nms.vector();
                output.write((char*)&num, sizeof(int));
                output.write((char*)normals.data(), static_cast<long>(num * sizeof(vec3_)));
            }
            else {
                int dummy = 0;
                output.write((char*)&dummy, sizeof(int));
