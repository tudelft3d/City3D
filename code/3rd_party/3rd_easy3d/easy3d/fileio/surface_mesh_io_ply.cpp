	...
	namespace io {
		...
		bool load_ply(const std::string& file_name, SurfaceMesh* mesh)
		{
            ...
            if (element_vertex) {// add vertex properties
                ...
                internal::add_vertex_properties<vec3_>(mesh, element_vertex->vec3_properties);
                internal::add_vertex_properties<vec2_>(mesh, element_vertex->vec2_properties);
            ...
            for (std::size_t i=0; i<face_vertex_indices.size(); ++i) {
                ...
                if (face.is_valid() && prop_texcoords) {
                    ...
                    if (face_texcoords.size() == vts.size() * 2) { // 2 coordinates per vertex
                        ...
                        do {
                            prop_texcoords[cur] = vec2_(face_texcoords[texcord_idx], face_texcoords[texcord_idx + 1]);
                            texcord_idx += 2;
                            cur = mesh->next(cur);
                        } while (cur != begin);
                    }
			...
			for (const auto& e : elements) {
                if (e.name == "vertex")
                    ...
                    internal::add_face_properties<vec3_>(mesh, e.vec3_properties);
                    internal::add_face_properties<vec2_>(mesh, e.vec2_properties);
                    ...
                    internal::add_edge_properties<vec3_>(mesh, e.vec3_properties);
                    internal::add_edge_properties<vec2_>(mesh, e.vec2_properties);
            ...
            if (Translator::instance()->status() == Translator::TRANSLATE_USE_FIRST_POINT) {
                auto& points = mesh->get_vertex_property<vec3_>("v:point").vector();

                // the first point
                const vec3_ p0 = points[0];
		...
		bool save_ply(const std::string& file_name, const SurfaceMesh* mesh, bool binary) {
            ...
            auto texcoord = mesh->get_halfedge_property<vec2_>("h:texcoord");
            if (texcoord) {
                FloatListProperty face_halfedge_texcoords;
