        ...
        bool load_obj(const std::string &file_name, SurfaceMesh *mesh) {
            ...
            if (Translator::instance()->status() == Translator::DISABLED) {
                for (std::size_t v = 1; v < fom->position_count; ++v)
                    builder.add_vertex(vec3_(fom->positions + v * 3));
                ...
                for (std::size_t v = 1; v < fom->position_count; ++v) {
                    ...
                    builder.add_vertex(vec3_(static_cast<float>(data[0] - origin.x), static_cast<float>(data[1] - origin.y), static_cast<float>(data[2] - origin.z)));
                }
                    ...
                    if (face.is_valid()) {
                        ...
                        if (prop_texcoords && texcoord_ids.size() == vertices.size()) {
                            ...
                            do {
                                ...
                                prop_texcoords[cur] = vec2_(fom->texcoords + 2 * tid);
                                cur = mesh->next(cur);
                            } while (cur != begin);
                        }
                        ...
                        if (prop_face_color) {
                            ...
                            prop_face_color[face] = vec3_(mat.Kd); // current implementation of easy3d uses only diffuse
                        }
                    }
                }
            }
...
namespace easy3d {
    ...
    namespace io {
        ...
        bool load_obj(const std::string &file_name, SurfaceMesh *mesh) {
            ...
            // for each texcoord
            std::vector<vec2_> texcoords;
            for (std::size_t v = 0; v < attrib.texcoords.size(); v += 2) {
                texcoords.emplace_back(vec2_(attrib.texcoords.data() + v));
            }
            ...
            for (std::size_t v = 0; v < attrib.vertices.size(); v += 3) {
                ...
                builder.add_vertex(vec3_(attrib.vertices.data() + v));
            }
#endif

            // create texture coordinate property if texture coordinates present
            if (!texcoords.empty())
                mesh->add_halfedge_property<vec2_>("h:texcoord");
            auto prop_texcoords = mesh->get_halfedge_property<vec2_>("h:texcoord");
            ...
            // now the material
            if (!materials.empty()) {
                auto face_color = mesh->add_face_property<vec3_>("f:color");
                int face_idx = 0;
                for (std::size_t i = 0; i < shapes.size(); i++) {
                    for (std::size_t f = 0; f < shapes[i].mesh.num_face_vertices.size(); f++) {
                        ...
                        if (face.is_valid()) {
                            ...
                            // ambient = vec3_(mat.ambient);
                            // specular = vec3_(mat.specular);
                            face_color[face] = vec3_(mat.diffuse);
                        }
                        ++face_idx;
                    }
                }
...
namespace easy3d {
    ...
    namespace io {
        ...
        bool load_obj(const std::string& file_name, SurfaceMesh* mesh)
        {
            ...
            float  x, y, z;
            std::vector<SurfaceMesh::Vertex>  vertices;
            std::vector<vec2_> all_tex_coords;   //individual texture coordinates
            std::vector<int> halfedge_tex_idx; //texture coordinates sorted for halfedges
            SurfaceMesh::HalfedgeProperty <vec2_> tex_coords = mesh->halfedge_property<vec2_>("h:texcoord");
            ...
            while (in && !feof(in) && fgets(s, 200, in))
            {
                ...
                else if (strncmp(s, "vt ", 3) == 0)
                {
                    ...
                    {
                        all_tex_coords.emplace_back(vec2_(x, y));
                    }
                }
            ...
            while (!tok.atEOF())
            {
                ...
                case 'v':
                    ...
                    case 't': //texcoord, 2 or 3 components
                        ...
                        match = tok.getTokenFloatArray(val, 3);
                        all_tex_coords.emplace_back(vec2_(val[0], val[1]));
                        tex3Comp |= (match == 3);
    ...
    namespace io {
        ...
        bool save_obj(const std::string &file_name, const SurfaceMesh *mesh) {
            ...
            if (trans) { // has translation
                ...
                auto points = mesh->get_vertex_property<vec3_>("v:point");
                for (auto vit = mesh->vertices_begin(); vit != mesh->vertices_end(); ++vit) {
                    const vec3_ &p = points[*vit];
            ...
            //normals
            auto normals = mesh->get_vertex_property<vec3_>("v:normal");
            if (normals) {
                for (auto vit = mesh->vertices_begin(); vit != mesh->vertices_end(); ++vit) {
                    const vec3_ &n = normals[*vit];
                    out << "vn " << n << "\n";
                }
            }
            ...
            //if so then add
            if (with_tex_coord) {
                auto tex_coord = mesh->get_halfedge_property<vec2_>("h:texcoord");
                for (auto hit = mesh->halfedges_begin(); hit != mesh->halfedges_end(); ++hit) {
                    const vec2_ &tex = tex_coord[*hit];
                    out << "vt " << tex << "\n";
                }
            }
