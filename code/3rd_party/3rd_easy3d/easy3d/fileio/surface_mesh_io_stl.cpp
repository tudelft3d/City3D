...
namespace easy3d {
	...
	namespace io {
		...
		bool load_stl(const std::string& file_name, SurfaceMesh* mesh)
		{
			...
			std::map<vec3_, SurfaceMesh::Vertex, CompareVec>            vMap(comp);
			std::map<vec3_, SurfaceMesh::Vertex, CompareVec>::iterator  vMapIt;
		...
		bool save_stl(const std::string& file_name, const SurfaceMesh* mesh)
		{
			...
			auto fnormals = mesh->get_face_property<vec3_>("f:normal");
			if (!fnormals)
			{
				fnormals = const_cast<SurfaceMesh*>(mesh)->add_face_property<vec3_>("f:normal");
