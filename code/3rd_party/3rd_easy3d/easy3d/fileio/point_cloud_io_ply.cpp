...
namespace easy3d {
	...
	namespace io {
		...
		bool load_ply(const std::string& file_name, PointCloud* cloud) {
            ...
            for (const auto& e : elements) {
                if (e.name == "vertex") {
                    internal::add_properties<vec3_>(cloud, e.vec3_properties);
                    internal::add_properties<vec2_>(cloud, e.vec2_properties);
            ...
            if (Translator::instance()->status() == Translator::TRANSLATE_USE_FIRST_POINT) {
                auto& points = cloud->get_vertex_property<vec3_>("v:point").vector();
