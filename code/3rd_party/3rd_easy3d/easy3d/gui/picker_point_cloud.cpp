...
namespace easy3d {
    ...
    PointCloud::Vertex PointCloudPicker::pick_vertex_cpu(PointCloud* model, int px, int py) {
        const std::vector<vec3_>& points = model->points();
...
#pragma omp parallel for
        for (int i = 0; i < num; ++i) {
            ...
            x = 0.5f * x + 0.5f;
            y = 0.5f * y + 0.5f;
            if (distance2(vec2_(x, y), vec2_(static_cast<float>(px), static_cast<float>(py))) < sqr_dist_thresh) {
                status[i] = 1;
