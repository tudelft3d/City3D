...
namespace easy3d {
    ...
    SurfaceMeshPicker::pick_vertex(SurfaceMesh *model, SurfaceMesh::Face picked_face, int x, int y) {
        ...
        const vec3_& point = picked_point(model, picked_face, x, y);
        ...
        const vec3_ &p = model->position(closest_vertex);
        const vec2_ q(project(p));
        const float sqr_dist = distance2(q, vec2_(static_cast<float>(x), static_cast<float>(y)));
    ...
    SurfaceMeshPicker::pick_edge(SurfaceMesh *model, SurfaceMesh::Face picked_face, int x, int y) {
        ...
        for (auto h : model->halfedges(picked_face)) {
            const vec3_ &s = model->position(model->source(h));
            const vec3_ &t = model->position(model->target(h));
        ...
        const vec3_ &s = model->position(model->source(closest_edge));
        const vec3_ &t = model->position(model->target(closest_edge));
        const Segment2 seg(vec2_(project(s)), vec2_(project(t)));
        const float sqr_dist = seg.squared_distance(vec2_(static_cast<float>(x), static_cast<float>(y)));
        if (sqr_dist < static_cast<float>(hit_resolution_ * hit_resolution_))
            return closest_edge;
        else
            return SurfaceMesh::Halfedge();
    ...
    vec3_ SurfaceMeshPicker::picked_point(SurfaceMesh *model, SurfaceMesh::Face face, int x, int y) const {
    ...
    SurfaceMesh::Face SurfaceMeshPicker::pick_face_cpu(SurfaceMesh *model, int x, int y) {
        ...
        const vec3_ &p_near = unproject(x, y, 0);
        const vec3_ &p_far = unproject(x, y, 1);
...
#pragma omp parallel for
    ...
    std::vector<SurfaceMesh::Face> SurfaceMeshPicker::pick_faces(SurfaceMesh *model, const Rect& rect) {
        ...
        const auto &points = model->get_vertex_property<vec3_>("v:point").vector();
...
#pragma omp parallel for
        for (int i = 0; i < num; ++i) {
            ...
            if (x >= xmin && x <= xmax && y >= ymin && y <= ymax) {
                if (geom::point_in_polygon(vec2_(x, y), region))
