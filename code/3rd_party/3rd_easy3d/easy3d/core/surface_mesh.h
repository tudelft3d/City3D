...
namespace easy3d {
    ...
    public: //----------------------------------------------- add new vertex / face
        ...
        Vertex add_vertex(const vec3_& p) { Vertex v = new_vertex(); vpoint_[v] = p; return v; }
    ...
    public: //--------------------------------------------- higher-level operations
         ...
         */
        Vertex split(Face f, const vec3_& p) { Vertex v=add_vertex(p); split(f, v); return v; }
         ...
         */
        Halfedge split(Edge e, const vec3_& p) { return split(e, add_vertex(p)); }
         ...
         */
        Halfedge insert_vertex(Edge e, const vec3_& p)
        {
    ...
    public: //------------------------------------------ geometry-related functions
        ...
        const vec3_& position(Vertex v) const { return vpoint_[v]; }

        /// position of a vertex
        vec3_& position(Vertex v) { return vpoint_[v]; }

        /// vector of vertex positions (read only)
        const std::vector<vec3_>& points() const override { return vpoint_.vector(); }

        /// vector of vertex positions
        std::vector<vec3_>& points() override { return vpoint_.vector(); }

        /// compute face normals by calling compute_face_normal(Face) for each face.
        void update_face_normals();

        /// compute normal vector of face \c f.
        vec3_ compute_face_normal(Face f) const;
        ...
        vec3_ compute_vertex_normal(Vertex v) const;
    ...
    private: //------------------------------------------------------- private data
        ...
        VertexProperty<vec3_>  vpoint_;
        VertexProperty<vec3_>  vnormal_;
        FaceProperty<vec3_>    fnormal_;
