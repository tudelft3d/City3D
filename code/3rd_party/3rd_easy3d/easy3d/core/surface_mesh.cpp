...
namespace easy3d {
    ...
    SurfaceMesh::SurfaceMesh()
    {
        ...
        vpoint_   = add_vertex_property<vec3_>("v:point");
    ...
    SurfaceMesh& SurfaceMesh::operator=(const SurfaceMesh& rhs)
    {
        if (this != &rhs)
        {
            ...
            vpoint_   = vertex_property<vec3_>("v:point");

            // normals might be there, therefore use get_property
            vnormal_  = get_vertex_property<vec3_>("v:normal");
            fnormal_  = get_face_property<vec3_>("f:normal");
    ...
    bool SurfaceMesh::read(const std::string &file_name)
    {
        ...
        auto point = vertex_property<vec3_>("v:point");
        ...
        input.read((char*)point.data(), static_cast<long>(nv * sizeof(vec3_)                             ));

        bool has_colors = false;
        input.read((char*)&has_colors, sizeof(bool));
        if (has_colors) {
            SurfaceMesh::VertexProperty<vec3_> color = vertex_property<vec3_>("v:color");
            input.read((char*)color.data(), static_cast<long>(nv * sizeof(vec3_)));
        }
    ...
    bool SurfaceMesh::write(const std::string& file_name) const
    {
        ...
        auto point = get_vertex_property<vec3_>("v:point");
        ...
        output.write((char*)point.data(), static_cast<long>(nv * sizeof(vec3_)));

        // check for colors
        auto color = get_vertex_property<vec3_>("v:color");
        bool has_colors = color;
        output.write((char*)&has_colors, sizeof(bool));
        if (has_colors)
            output.write((char*)color.data(), static_cast<long>(nv * sizeof(vec3_)));
    ...
    void SurfaceMesh::clear()
    {
        ...
        vnormal_  = VertexProperty<vec3_>();
        fnormal_  = FaceProperty<vec3_>();
    }
    ...
    {
        if (!fnormal_)
            fnormal_ = face_property<vec3_>("f:normal");
    ...
    vec3_ SurfaceMesh::compute_face_normal(Face f) const
    {
        Halfedge h = halfedge(f);
        Halfedge hend = h;

        vec3_ p0 = vpoint_[target(h)];
        h = next(h);
        vec3_ p1 = vpoint_[target(h)];
        h = next(h);
        vec3_ p2 = vpoint_[target(h)];
    ...
    {
        if (!vnormal_)
            vnormal_ = vertex_property<vec3_>("v:normal");
        ...
        auto angle_weighted_face_normals = [this](Vertex v) -> vec3_ {
            vec3_     nn(0, 0, 0);
            Halfedge  h = out_halfedge(v);
    ...
    vec3_ SurfaceMesh::compute_vertex_normal(Vertex v) const
    {
        vec3_     nn(0, 0, 0);
        Halfedge  h = out_halfedge(v);
    ...
    void SurfaceMesh::stitch(Halfedge h0, Halfedge h1) {
        ...
        const vec3_& p_org0 = geom::barycenter(vpoint_[org0], vpoint_[dest1]);
        const vec3_& p_org1 = geom::barycenter(vpoint_[dest0], vpoint_[org1]);
