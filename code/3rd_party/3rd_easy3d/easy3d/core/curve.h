...
namespace easy3d {
    ...
    namespace curve {
         ...
         *     curve->set_uniform_coloring(vec3_(0, 0, 1, 1));
         ...
         *     curve->set_impostor_type(easy3d::LinesDrawable::CYLINDER);
         *     curve->set_line_width(5);
         *     curve->set_uniform_coloring(vec3_(0, 1, 0, 1));
         *     viewer.add_drawable(curve);
         * }
         * \endcode
         */
     ...
     * Example:
     *  \code
     *      auto cv = new easy3d::curve::Bezier<vec3_>;
     *      curve->set_steps(num);
     ...
     * Example:
     *  \code
     *      auto cv = new easy3d::curve::BSpline<vec3_>;
     *      curve->set_steps(num);
     *      for (const auto& p : points)
     *      cv->add_way_point(p);
     *      for (int i = 0; i < cv->node_count(); ++i)
     ...
     * Example:
     *  \code
     *      auto cv = new easy3d::curve::CatmullRom<vec3_>;
     *      curve->set_steps(num);
     *      for (const auto& p : points)
     *      cv->add_way_point(p);
     *      for (int i = 0; i < cv->node_count(); ++i)
