...
namespace easy3d {
    ...
    {
        auto fnormal = face_property<vec3_>("f:normal");
    ...
    vec3_ PolyMesh::compute_face_normal(Face f) const
    {
        const auto& vts = vertices(f);

        if (vts.size() == 3) { // face is a triangle
            const vec3_& p0 = vpoint_[vts[0]];
            const vec3_& p1 = vpoint_[vts[1]];
            const vec3_& p2 = vpoint_[vts[2]];
            ...
            for (std::size_t id = 0; id < vts.size(); ++id) {
                ...
                vec3_ p0 = vpoint_[vts[id_prev]];
                vec3_ p1 = vpoint_[vts[id]];
                vec3_ p2 = vpoint_[vts[id_next]];
    ...
    {
        auto vnormal = vertex_property<vec3_>("v:normal");

        // always re-compute face normals
        update_face_normals();
        auto fnormal = get_face_property<vec3_>("f:normal");
