...
namespace easy3d {
    ...
    typedef Vec<2, float> vec2_;
    /// \brief A 3D point/vector of \p float type.
    typedef Vec<3, float> vec3_;
    ...
    namespace geom {
         ...
         */
        inline bool point_in_polygon(const vec2_ &p, const std::vector<vec2_> &polygon);
        ...
        inline float triangle_area(const vec3_ &p1, const vec3_ &p2, const vec3_ &p3) {
            return 0.5f * length(cross(p2 - p1, p3 - p1));
        }

        /** \brief Computes signed area of a triangle given by three points. */
        inline float triangle_signed_area(const vec2_ &p1, const vec2_ &p2, const vec2_ &p3) {
            return 0.5f * det(p2 - p1, p3 - p1);
        }

        /** \brief Computes the normal vector of a triangle given by three points. */
        inline vec3_ triangle_normal(const vec3_ &p1, const vec3_ &p2, const vec3_ &p3) {
            vec3_ n = cross(p2 - p1, p3 - p2);
            return normalize(n);
        }

        /** \brief Computes the distance of a point p to a line segment given by vec3s (v0,v1). */
        inline float dist_point_line_segment(const vec3_ &p, const vec3_ &v0, const vec3_ &v1, vec3_ &nearest_point);

        /** \brief Computes the distance of a point p to the triangle given by vec3s (v0, v1, v2). */
        inline float
        dist_point_triangle(const vec3_ &p, const vec3_ &v0, const vec3_ &v1, const vec3_ &v2, vec3_ &nearest_point);

        /** \brief Computes the circum center of a tetrahedron. */
        inline vec3_ tetra_circum_center(const vec3_& p, const vec3_& q, const vec3_& r, const vec3_& s);
    ...
    namespace geom {
        ...
        inline bool point_in_polygon(const vec2_ &p, const std::vector<vec2_> &polygon) {
            ...
            for (std::size_t i = 0, j = n - 1; i < n; j = i, ++i) {
                const vec2_ &u0 = polygon[i];
                const vec2_ &u1 = polygon[j];  // current edge
        ...
        dist_point_triangle(const vec3_ &p, const vec3_ &v0, const vec3_ &v1, const vec3_ &v2, vec3_ &nearest_point) {
            vec3_ v0v1 = v1 - v0;
            vec3_ v0v2 = v2 - v0;
            vec3_ n = cross(v0v1, v0v2); // not normalized !
