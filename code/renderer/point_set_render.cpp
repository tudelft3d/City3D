...
void PointSetRender::draw_point_set_per_point_color(PointSet* pset) {
	...
	double* points = &(pset->points()[0].x);
	double* colors = &(pset->colors()[0].x);

	glPointSize(point_set_style_.size);

	if (pset->has_normals()) {
		double* normals = &(pset->normals()[0].x);
...
void PointSetRender::draw_vertex_groups(PointSet* pset) {
	...
	const std::vector<VertexGroup::Ptr>& fathers = pset->groups();
	if (fathers.empty())
		return;
	...
	for (std::size_t i = 0; i < fathers.size(); ++i) {
		VertexGroup* g = fathers[i];
		if (g->is_visible()) {
			const std::vector<VertexGroup::Ptr>& children = g->children();
			if (children.empty())
				groups.push_back(g);
			else 
				groups.insert(groups.begin(), children.begin(), children.end());
		}
	}
