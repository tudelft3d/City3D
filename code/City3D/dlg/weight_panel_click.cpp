	...
	, triangle_(3)
{
    ...
    pos_height_ = QPointF(80 + x_offset, 180);
	pos_complexity_ = QPointF(250 + x_offset, 180);

	triangle_[0] = pos_fitting_;
	triangle_[1] = pos_height_;
	triangle_[2] = pos_complexity_;

	pos_ = pointFromWeights(pos_fitting_, pos_height_, pos_complexity_, Method::lambda_data_fitting, Method::lambda_model_height, Method::lambda_model_complexity);

	fitting_ = truncate_digits(Method::lambda_data_fitting, 3);
	coverage_ = truncate_digits(Method::lambda_model_height, 3);
...
void WeightPanelClick::paintEvent(QPaintEvent* event) {
	...
	QString text_coverage = QString("Height (%1)").arg(coverage_);		painter.drawText(pos_height_.x() - 40, pos_height_.y() + 20, text_coverage);
...
void WeightPanelClick::mouseMoveEvent(QMouseEvent* event)
{
	...
	if (triangle_.containsPoint(p, Qt::OddEvenFill)) {
		float area_sum = area_of_triangle(pos_fitting_, pos_height_, pos_complexity_);

		float area_data_fitting = area_of_triangle(pos_complexity_, pos_height_, p);
		float area_model_height = area_of_triangle(pos_complexity_, pos_fitting_, p);
		float area_model_complexity = area_of_triangle(pos_fitting_, pos_height_, p);

		float fitting = truncate_digits(area_data_fitting / area_sum, 3);
		float coverage = truncate_digits(area_model_height / area_sum, 3);
		...
		std::string weights = 
			...
			"; Height: " + str_coverage.substr(0, str_coverage.find(".") + 4) +
...
void WeightPanelClick::computeWeight() {
	...
	float area_data_fitting = area_of_triangle(pos_complexity_, pos_height_, pos_);
	float area_model_coverage = area_of_triangle(pos_complexity_, pos_fitting_, pos_);
	float area_model_complexity = area_of_triangle(pos_fitting_, pos_height_, pos_);

	Method::lambda_data_fitting = area_data_fitting / area_sum;
	Method::lambda_model_height = area_model_coverage / area_sum;
...
void WeightPanelClick::updateUI() {
	float sum = Method::lambda_data_fitting + Method::lambda_model_height + Method::lambda_model_complexity;
	fitting_ = truncate_digits(Method::lambda_data_fitting / sum, 3);
	coverage_ = truncate_digits(Method::lambda_model_height / sum, 3);
