...
namespace MapTypes {
	...
	*/
	...
	public:
		...
		const std::vector<const Facet*>& holes() const { return holes_; }
		void add_hole(const Facet* hole);
		void remove_hole(const Facet* hole);
		void clear_holes() { holes_.clear(); }

		// current facet will make a hole in the return facet
		Facet* hole_holder() const { return hole_holder_; }
		void   set_hole_holder(Facet* f);
