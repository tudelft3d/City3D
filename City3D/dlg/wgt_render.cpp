...
void WgtRender::setPerFaceColor(bool b) {
	Tessellator::set_per_face_color(b);
	mainWindow_->canvas()->update();
}


void WgtRender::setShowSurface(bool b) {
	SurfaceStyle s = Tessellator::surface_style();
	s.visible = b;
	Tessellator::set_surface_style(s);
	mainWindow_->canvas()->update();
}


void WgtRender::setShowSharpEdges(bool b) {
	EdgeStyle s = MeshRender::sharp_edge_style();
	s.visible = b;
	MeshRender::set_sharp_edge_style(s);
	mainWindow_->canvas()->update();
}
...
void WgtRender::updatePanel() {
	const SurfaceStyle& ss = Tessellator::surface_style();
	checkBoxSurface->setChecked(ss.visible);

	const EdgeStyle& cs = MeshRender::sharp_edge_style();
	checkBoxSharpEdges->setChecked(cs.visible);
	checkBoxPerFaceColor->setChecked(Tessellator::per_face_color());
