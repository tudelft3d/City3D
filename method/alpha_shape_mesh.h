...
public:
	...
	static Map* apply(AlphaShape* as, const Plane3d& plane, double radius);

	// the input is a vertex group
	static Map* apply(const VertexGroup* g, double radius);

	// the input is a subset of a point cloud and the points lie on plane
	static Map* apply(const PointSet* pset, const std::vector<unsigned int>& point_indices, const Plane3d& plane, double radius);
