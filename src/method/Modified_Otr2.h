...
#include "./OTR_2/Reconstruction_triangulation_2.h"
#include "./OTR_2/Reconstruction_edge_2.h"
	 ...
	 public:
		...
		bool do_collapse(Edge edge) {
			...
			for (Finite_edges_iterator ei = m_dt.finite_edges_begin(); ei != m_dt.finite_edges_end(); ++ei)
			{
				...
				Segment segment = m_traits.construct_segment_2_object()(
					source->point(), target->point());
				FT min_dist = 1e30;
				for (std::size_t vi = 0; vi < input_points.size(); ++vi)
				{
					Point vp = input_points[vi];
					FT vp_dist = m_dt.compute_distance2(vp, segment);
					if (vp_dist < min_dist)
						min_dist = vp_dist;
				}
			...
			if (max_d > m_dis)
			{
//		std::cout<<"this is nan case"<<std::endl;
//		return false ;
			}
		...
		bool is_above_distance(const Rec_edge_2& pedge)
		{
			FT m_dis = 0.01;
			FT min_dist = 1e30;
			Segment segment = m_dt.get_segment(pedge.edge());
			for (std::size_t vi = 0; vi < input_points.size(); ++vi)
			{
				Point vp = input_points[vi];
				FT vp_dist = m_dt.compute_distance2(vp, segment);
...
//			std::cout<<"this is a case"<<std::endl;
		...
		*/
		 ...
		 */
		bool run(const unsigned int steps,double tolerance) {
			m_tolerance = (FT) tolerance;
			CGAL::Real_timer timer;
			if (m_verbose > 0)
