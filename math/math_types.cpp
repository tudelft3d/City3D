...
namespace Geom {
	...
	void sort_along_direction(const std::vector<vec3>& points, const vec3& dir, std::vector<unsigned int>& sorted_indices)
	{
		...
		typedef Compare::ElementAndValue<unsigned int>		IndexAndValue;
		typedef Compare::ElementAndValueLess<unsigned int>	IndexAndValueLess;

		std::vector<IndexAndValue> temp;

		const vec3& v = points[0]/* - vec3(0, 0, 0)*/;
		double dot_prod = dot(v, dir);
		temp.push_back(IndexAndValue(0, dot_prod));
		for (unsigned int i = 1; i < points.size(); ++i) {
			const vec3& vec = points[i]/* - vec3(0, 0, 0)*/;
			dot_prod = dot(vec, dir);
			temp.push_back(IndexAndValue(i, dot_prod));
		}

		std::sort(temp.begin(), temp.end(), IndexAndValueLess());

		sorted_indices.clear();
		for (unsigned int i = 0; i < temp.size(); ++i) {
			sorted_indices.push_back(temp[i].element_);
		}
	}
