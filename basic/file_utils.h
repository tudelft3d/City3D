...
namespace FileUtils {
	...
	bool		BASIC_API delete_directory(const std::string& path); // all content will be deleted first
	bool		BASIC_API delete_contents(const std::string& path);	 // delete contents only

	std::string BASIC_API parent_directory(const std::string& path);
	...
	*/
	...
	std::string BASIC_API FootPrint_resource_directory();
}
